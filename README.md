...
- Recommended practical next step is MacBook/Xcode physical-device verification of `Phase 21-A4 - On-device Live Framing Hint UX Pass`. Confirm category icons, expanded `Next hint` cycling, disabled single-hint action state, local-only behavior, filter preview, selfie mirror parity, flash behavior, and focal crop. Phase 21-C-R3-RUN remains available only after the exact approval phrase in `docs/phase-21-c-r3-depth-anything-operator-artifact-verification-and-physical-device-benchmark-approval-request.md` is provided, a local ignored artifact is ready, and MacBook/Xcode physical-device benchmarking is available. Any model artifact handling, Core ML package use, inference execution, benchmark run, Camera runtime integration beyond explicit local scope, upload path, provider/cloud call, iOS provider/model key, raw artifact, sensitive inference, raw frame persistence, or production rollout requires separate explicit approval.
