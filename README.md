...
- PT2-SF-R8 adds a docs-only approval gate for a future DEBUG Filter Lab provider-backed one-reference smoke. It runs no provider/model call, reads/prints/commits no API key, changes no Swift/backend runtime, expands no payload, and requires a separate exact R8-RUN approval phrase before any app-driven provider smoke. The future smoke may send only the style reference image through the backend; the original/apply image must stay local, and `productionReady:false` remains locked.
