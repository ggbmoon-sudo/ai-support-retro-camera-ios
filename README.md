...
- Recommended next step is Phase 21-C-R3-RUN: Approved Depth Anything V2 Small Physical-device Benchmark, only if the exact approval phrase in `docs/phase-21-c-r3-depth-anything-operator-artifact-verification-and-physical-device-benchmark-approval-request.md` is provided and local ignored artifact prerequisites are ready. Any model artifact handling, Core ML package use, inference execution, benchmark run, Camera runtime integration, upload path, provider/cloud call, iOS provider/model key, raw artifact, sensitive inference, or production rollout requires that separate explicit approval.
