...
- Phase OD-R8F adds a backend/local-only Parameter-pack Candidate Builder. Use `npm run qa:aesthetic-parameters:pack-candidate-builder` to build a review-only candidate scaffold from only the OD-R8E accepted candidates: `horizonAngle.softTiltThreshold` and `visualWeightMoment.balanceWarningPolicy`. It excludes `headroomRatio.highSoftWarningThreshold` and `sharpnessRatio.lowConfidenceThreshold` as `needs_more_data`, keeps `candidateOnly:true`, `eligibleForParameterPackExport:false`, `eligibleForAppRuntime:false`, `parameterPackExported:false`, `appRuntimeWritePerformed:false`, and `productionReady:false`, and adds no image read, CV inference, report write, parameter pack export, iOS/Camera runtime change, provider/cloud/Xiaoyi relay call, network/upload, Swift/Xcode change, model artifact, training/fine-tuning, app runtime transfer, or production rollout.
- Phase OD-R8E adds a backend/local-only Reviewed Candidate Acceptance Gate. Use `npm run qa:aesthetic-parameters:candidate-acceptance-gate` to apply manual review decisions to OD-R8D review-only candidates: two are accepted for future pack-candidate review and two are marked `needs_more_data`. It keeps `eligibleForParameterPackExport:false`, `eligibleForAppRuntime:false`, `parameterPackExported:false`, `appRuntimeWritePerformed:false`, and `productionReady:false`, and adds no image read, CV inference, report write, parameter pack export, iOS/Camera runtime change, provider/cloud/Xiaoyi relay call, network/upload, Swift/Xcode change, model artifact, training/fine-tuning, app runtime transfer, or production rollout.
- Phase OD-R8D adds a backend/local-only Parameter Tuning Dry-run From Bridge. Use `npm run qa:aesthetic-parameters:tuning-dry-run-from-bridge` to consume the sanitized OD-R8C bridge output and produce review-only parameter adjustment candidates. It keeps every adjustment `candidateOnly:true`, keeps `eligibleForParameterPackExport:false`, `eligibleForAppRuntime:false`, `parameterPackExported:false`, `appRuntimeWritePerformed:false`, and `productionReady:false`, and adds no image read, CV inference, report write, parameter pack export, iOS/Camera runtime change, provider/cloud/Xiaoyi relay call, network/upload, Swift/Xcode change, model artifact, training/fine-tuning, app runtime transfer, or production rollout.
- Phase OD-R8C adds a backend/local-only Parameter Tuning Aggregation Bridge. Use `npm run qa:aesthetic-parameters:tuning-aggregation-bridge` to convert the sanitized OD-R7F-R2 five-fixture aggregation summary into internal tuning review signals for a future OD-R8D dry-run. It keeps `eligibleForParameterTuningDryRun:true` only for bridge review while keeping `eligibleForParameterPackExport:false`, `eligibleForAppRuntime:false`, and `productionReady:false`. It reads no images, runs no CV inference, writes no reports, creates no production parameter pack, changes no iOS/Camera runtime, and adds no provider/cloud/Xiaoyi relay call, network/upload, Swift/Xcode change, model artifact, training/fine-tuning, app runtime transfer, or production rollout.
- Phase OD-R7E adds a backend/local-only expected range comparison for sanitized OD-R7D calibration feature output. The default CLI compares inline sanitized `calibration_001` values only, reads no images, runs no CV inference, emits soft warning buckets rather than scores, writes no reports, and adds no real photo/local config/generated report commit, cloud/provider/Xiaoyi relay call, network call, upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, app runtime transfer, Camera behavior change, or production rollout. `productionReady:false` remains locked.
- Phase OD-R7F adds a backend/local-only Local CV calibration multi-fixture aggregation gate. Use `npm run qa:aesthetic-local-cv:calibration-aggregation` to aggregate sanitized OD-R7E expected-range outputs into warning frequency and feature status buckets without reading images by default. The current inline fixture count is `1`, so `insufficientSampleSize:true` blocks parameter tuning handoff; the gate emits no score/rating/aesthetic grade and adds no provider/cloud/Xiaoyi relay call, network/upload, real image read by default, generated report, Swift/Xcode change, model artifact, training/fine-tuning, app runtime transfer, or production rollout.
- Phase OD-R7F-R2 updates the Local CV aggregation gate to use five manually reviewed sanitized OD-R7F-R1 feature vectors by default. `npm run qa:aesthetic-local-cv:calibration-aggregation` now reports `fixtureCount:5` and `insufficientSampleSize:false` for first aggregation review while keeping `eligibleForParameterTuning:false`, `eligibleForAppRuntime:false`, and `productionReady:false`. It also regression-tests safe fixture-root handling for the OD-R7D smoke CLI. No image read by default, committed fixture/config/report, provider/cloud/network/upload, Swift/Xcode change, model artifact, tuning/runtime handoff, or production rollout is added.
- Phase OD-R7D adds a backend/local-only ignored local fixture calibration smoke for OD-R7B/OD-R7C. The default CLI is safely blocked unless an ignored local config and exactly one approved ignored fixture token `calibration_001` exist. When gates pass it may read that one ignored local fixture and emit sanitized in-memory feature buckets only. It writes no reports and adds no real photo/local config/generated report commit, cloud/provider/Xiaoyi relay call, network call, upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, app runtime transfer, Camera behavior change, or production rollout. `productionReady:false` remains locked.
- Phase OD-R7C adds a backend/local-only Local CV calibration fixture gate for the OD-R7B extractor. It validates disabled ignored-local fixture policy and approved synthetic/ignored fixture-token shape before any real image read or calibration smoke. The default CLI is `fixture_gate_no_image_read`, safely blocked, and makes no real image read, CV inference, cloud/provider/Xiaoyi relay call, network call, upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, app runtime transfer, real photo/local config/generated report commit, or production rollout. `productionReady:false` remains locked.
- Phase OD-R7B adds a backend/local-only Local CV Feature Extractor prototype. It maps safe inline synthetic local-CV observations into OD-R7A-compatible feature vectors for the local AI parameter-pack pipeline. It makes no real image read, CV inference, cloud/provider/Xiaoyi relay call, network call, image upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-P1A adds a backend-only App Transfer Preflight Gate. It validates a safe inline synthetic OD-R8B export candidate against app contract requirements before any future transfer is considered, while keeping runtime transfer blocked. It makes no provider/cloud call, API key use, network call, image read/upload, CV inference, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, Xcode project modification, app bundle write, model install, app runtime write, app transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R8B adds a backend-only Parameter Pack Export Gate. It validates a safe inline synthetic OD-R8A parameter-pack candidate as structurally exportable while keeping app runtime transfer blocked. It makes no provider/cloud call, API key use, network call, image read/upload, CV inference, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, app runtime write, app transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R8A adds a backend-only Parameter Tuning Harness dry-run. It converts reviewed synthetic parameter candidates and synthetic local CV benchmark signals into a structured versioned parameter-pack candidate while keeping app transfer blocked. It makes no real image read, CV inference, cloud/provider call, network call, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R7A adds a backend-only Local CV Feature Vector Benchmark dry-run. It evaluates safe inline synthetic numeric feature vectors against OD-R1 registry tags, OD-R6A parameter candidates, and OD-R6B review decisions. It makes no real image read, CV inference, cloud/provider call, network call, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R5C adds a backend-only opt-in Cloud Teacher provider smoke harness. The default CLI path is `blocked_no_network`, exits successfully when safely blocked, and makes no real provider/cloud AI call, network call, image read/upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, provider SDK, or production rollout. `productionReady:false` remains locked.
- Phase OD-R5B adds a backend-only, fail-closed Cloud Teacher provider sandbox readiness gate plus a redacted request-envelope builder for a future opt-in provider smoke. It makes no real provider/cloud AI call, network call, image read/upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R6B adds a backend-only Human Review Queue schema for reviewing teacher label candidates and parameter candidates before calibration, evaluation, tuning candidacy, benchmark planning, or app-transfer planning. It makes no real image read, cloud/provider call, network call, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R6A adds a backend-only Parameter Candidate Runner dry-run. It converts validated OD-R4/OD-R5A teacher stub labels into structured parameter candidates and aggregate counts for later review/tuning/benchmark planning. It makes no real image read, cloud/provider call, network call, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R5A adds a backend-only Cloud AI Teacher labeling sandbox preflight and stub adapter. It accepts sanitized OD-R3-style job plans, emits OD-R4 teacher-contract-shaped stub responses, and validates those stubs through the OD-R4 contract validator. It makes no real provider/cloud AI call, network call, image read/upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, app runtime transfer, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R4 adds a backend-only Cloud AI Teacher contract for future offline structured labeling. It defines structured request/response validation, sanitized CLI output, tests, and app-transfer-readiness fields while keeping app runtime transfer blocked. It makes no cloud provider call, network call, image read/upload, crawler/download, training/fine-tuning, iOS runtime integration, Swift file, model install, upload path, real dataset/photo/local config/generated report, or production rollout. `productionReady:false` remains locked.
- Phase OD-R3 adds a backend-only Parameter Mining Bot dry-run. It connects the OD-R1 Aesthetic Parameter Registry and OD-R2 Dataset / Source Manifest Schema, then emits sanitized label-job planning output with category/tag counts and blocker buckets. It reads only inline synthetic manifest objects; no crawler/download mode, image read, cloud teacher call, provider/model API call, training/fine-tuning, inference, iOS runtime integration, Swift file, model install, upload path, real dataset/photo/local config/generated report, or production rollout is added. `productionReady:false` remains locked.
- Phase OD-R2 adds the backend-only Dataset / Source Manifest Schema layer for OD-R. It defines safe source, image, and label-job manifest validation, a sanitized no-crawler/no-download/no-cloud-teacher CLI, and backend tests only; no Swift runtime file, model install, Core ML package, crawler/download mode, cloud teacher call, training, inference, Camera integration, upload path, provider/model key, real dataset/photo/local config/generated report, or production rollout is added. `productionReady:false` remains locked.
- Phase OD-R1 starts the OD-R research / parameter / dataset / fine-tune preparation track with a backend-only Aesthetic Parameter Registry. It adds structured photography error tags, symbolic feature/threshold/suppression/action keys, a sanitized validator CLI, and backend tests only; no Swift runtime file, model install, Core ML package, crawler/download mode, cloud teacher call, training, inference, Camera integration, upload path, provider/model key, dataset, local config, generated report, or production rollout is added. App/model installation belongs later to OD-P product integration after benchmark and safety gates pass. `productionReady:false` remains locked.
- Phase OD-03A adds the backend no-runtime Depth Anything V2 Small Core ML sandbox preflight gate. It adds a sanitized policy evaluator, no-network/no-model CLI, and backend tests only; no Swift runtime file, model file, Core ML package, model download, inference execution, benchmark run, Camera integration, preview-frame upload, upload payload change, Camera live cloud AI entry, provider/model call, API key, dataset, local config, generated report, or production rollout is added.
- Phase OD-03 records a docs-only Depth Anything V2 Small Core ML sandbox preflight plan. This commit adds no backend preflight gate/script/tests, no Swift runtime file, no model file, no Core ML package, no model download, no inference execution, no benchmark run, no Camera integration, no preview-frame upload, no upload payload change, no Camera live cloud AI entry, no provider/model call, no API key, and no production rollout. The local `ios-app/AIPhotoApp/Features/Camera/DepthAnythingV2SmallSandbox.swift` scaffold remains untracked for a later separate phase and must not be included in this docs-only commit.
...
- Recommended next step is Phase 21-Z2H-SF: SiliconFlow Latency Architecture Decision Gate. Any further provider API call, API key use beyond local ignored context, provider diagnosis, alternative model benchmark, image upload, model call, benchmark, RunPod provisioning, Qwen3 install/download/load/call, iOS integration, upload payload change, app-facing endpoint, production endpoint, or production rollout requires separate explicit user approval.
