...
- Recommended practical next step is Phase 21-A3-R1: Physical-device Live Guidance Tuning Feedback Pass, using the user's iPhone feedback to adjust on-device Vision thresholds again if needed. Phase 21-C-R3-RUN remains available only after the exact approval phrase in `docs/phase-21-c-r3-depth-anything-operator-artifact-verification-and-physical-device-benchmark-approval-request.md` is provided, a local ignored artifact is ready, and MacBook/Xcode physical-device benchmarking is available. Any model artifact handling, Core ML package use, inference execution, benchmark run, Camera runtime integration, upload path, provider/cloud call, iOS provider/model key, raw artifact, sensitive inference, or production rollout requires separate explicit approval.
