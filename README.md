...
PT2 now uses SiliconFlow as the active backend-only provider direction for internal/debug Photo Advisor and Filter Lab credential work, with `deepseek-ai/DeepSeek-V4-Flash` through the official OpenAI-compatible `/v1/chat/completions` path for text-only credential smoke. PT2-SF-R1 text-only smokes accepted, but PT2-SF-R2 image QA found the current DeepSeek-V4-Flash image request blocked with sanitized `provider_vision_request_rejected`; choose a SiliconFlow vision-capable model/path before Filter Lab structured recipe QA or iOS integration. Filter Lab receives a validated structured recipe only in later QA phases, not generated bitmaps, shader/code, LUT URLs, or rendering instructions. iOS still has no provider SDK, provider key, or direct SiliconFlow call; production/default behavior remains mock/local, and Camera remains local-only.
...
PT2 is still not a production rollout. It now uses backend-only SiliconFlow OpenAI-compatible internal/debug credential smoke work for Photo Advisor and Filter Lab with server-side secrets only. Text-only smoke accepted for `deepseek-ai/DeepSeek-V4-Flash`, while PT2-SF-R2 image QA is blocked by `provider_vision_request_rejected` for the current model/request path. No iOS provider key/direct provider call, Camera cloud AI entry, image editing provider, StoreKit/payment, or production rollout is added.
