...
- Phase 21-A starts the on-device live framing implementation track with Apple Vision geometry-only analysis for ephemeral preview frames. It adds typed local geometry/composition signal contracts, a Vision face/body/pose geometry analyzer, safe composition buckets, and a future overlay coordinate mapper. It adds no cloud live AI, no provider/model key, no direct iOS provider/model call, no model file, no frame upload, no backend/iOS upload payload change, no raw frame persistence, and no production rollout. The next recommended phase after commit/push is `Phase 21-B - AVFoundation Depth Capability Probe`.
