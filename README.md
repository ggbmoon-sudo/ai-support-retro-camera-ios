...
- Recommended practical next step is MacBook/Xcode physical-device verification of Phase 21-A3-R3. Confirm the Camera preview is no longer zoomed, the transparent focal framing box can be moved freely, the image inside the box is not live-magnified, the slider changes only box size/focal crop, and the captured result is only the area inside the box. Phase 21-C-R3-RUN remains available only after the exact approval phrase in `docs/phase-21-c-r3-depth-anything-operator-artifact-verification-and-physical-device-benchmark-approval-request.md` is provided, a local ignored artifact is ready, and MacBook/Xcode physical-device benchmarking is available. Any model artifact handling, Core ML package use, inference execution, benchmark run, Camera runtime integration beyond explicit local scope, upload path, provider/cloud call, iOS provider/model key, raw artifact, sensitive inference, or production rollout requires separate explicit approval.
