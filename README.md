...
- Local AI Compose P25-R5 trims the one-shot composition payload to a 768px metadata-free JPEG, low-detail vision input, 256-token JSON cap, deterministic temperature, and an 18-second backend / 20-second iOS fail-fast window. A sanitized real-provider latency probe measured the original path at about 13.7s and the trimmed non-streaming path at about 11.3s; SSE still took about 10.7s to first content and 12.1s to valid JSON, proving the remaining gap to the 2-second goal is upstream Xiaoyi `gpt-5.6-luna` latency rather than local upload or response buffering. Ring alignment now uses fresh accepted 15 FPS local Vision tracking geometry, a much narrower display-space hit radius, five-sample hold, near-zone jitter hysteresis, visible near/hold progress, and haptic stage confirmation. The frozen one-response contract, manual Camera control, DEBUG/internal boundary, and `productionReady:false` remain unchanged.
- Local AI Compose P25-R4 makes the staged workflow strictly one-shot. After explicit DEBUG consent, Camera uploads exactly one metadata-free keyframe through the backend, accepts one validated GPT subject box/composition position, freezes that subject and plan immediately, and ends cloud analysis. The animated reading state remains visible while that single request is in flight; every ring, reticle, composition-frame, zoom/distance, reacquisition, and Ready update afterward is local Apple Vision geometry only. There is no cadence loop, second keyframe, consensus replacement, automatic camera actuation, or production rollout, and `productionReady:false` remains locked.
- Local AI Compose P25-R3 replaces the unstable all-at-once overlay with a strict staged Live AI viewfinder: animated analysis of at most three sequential keyframes, two-match strategy consensus, one frozen GPT-grounded subject, a colored subject ring aimed at a fixed reticle, then one composition frame with local move/zoom/distance guidance and a latched Ready state. The first validated cloud box now replaces the center preflight candidate instead of inheriting it. Once a plan locks, cloud uploads stop for that session and later replies cannot replace the target; Apple Vision continues the same local image-space track. Consent remains explicit and DEBUG-only, no hardware zoom/capture is actuated, and `productionReady:false` remains locked.
- Local AI Compose P25-R2 makes AI Compose the default Camera mode and removes the hidden long-press prerequisite that left `Start Live AI Compose` disabled. In DEBUG, once Camera permission is ready, the app presents the Live AI session consent once; no keyframe is sent until the user accepts. Starting without a long press seeds a central provisional subject for the first Xiaoyi grounding keyframe, while long press remains an optional precise-subject override. Returning from a captured/imported photo re-enables local Compose. Cancelling consent does not repeatedly prompt, and manual Start remains available. Release still has no cloud entry, and all P25 cadence/privacy/control boundaries remain unchanged.
- Local AI Compose P25-R1 repairs the reported Xcode scope error in `CameraView`. The DEBUG Live AI status badge now lives in the outer Camera chrome `GeometryReader`, where the existing `topControlInset` is defined, instead of referencing that local value from the separate `cameraFullscreenCanvas` computed view. The badge keeps the intended safe-area position and does not intercept Camera touches. No session cadence, provider payload, tracking, capture, privacy, or rollout behavior changed.
- Local AI Compose P25 turns the explicitly approved DEBUG/internal Xiaoyi experiment into a bounded live-like composition session. After one session-specific consent and a long-press focus hint, Camera may send at most one metadata-stripped keyframe per second through the backend to `gpt-5.6-luna`, with exactly one request in flight and no queue or catch-up burst. Schema `1.1` requires a grounded subject kind/box plus a strict composition strategy; invalid, sensitive, scored, free-form, or focus-inconsistent output fails closed. Between cloud replies, Apple Vision tracks the chosen image-space subject at a nominal 15 FPS and the existing 2D AR-style overlay supplies move/zoom/distance guidance. This is HTTP keyframe polling inspired by Gemini Live, not a persistent bidirectional Gemini Live/WebSocket stream. Camera hardware and shutter stay photographer-controlled; no iOS provider key/direct call, raw cloud artifact persistence, training use, ARKit world anchor, release/default entry, or production rollout is added. `productionReady:false` remains locked.
- Local AI Compose P24 adds an explicitly approved DEBUG/internal hybrid composition planner. After Compose is enabled and the photographer long-presses a subject, a fresh consent sheet allows one metadata-stripped preview JPEG to travel through the backend to Xiaoyi `gpt-5.6-luna`. The model may return only one strict enum plan; invalid, sensitive, scored, free-form, or inconsistent output fails closed. That plan becomes one frozen local target, while P23 Vision tracking, arrows, Hold/Ready, preview, controls, and manual shutter continue on-device without further upload. There is no continuous video/frame streaming, auto-trigger, automatic camera control, iOS provider key/direct call, raw prompt/response/image logging or persistence, ARKit session, training use, or production rollout. `productionReady:false` remains locked.
- Local AI Compose P23 turns the existing long-press affordance into a meaningful True Subject Lock. The approximately 2 FPS full detector remains authoritative, uses tighter same-kind center/area/overlap association, retains two misses without translating the box, and enters reacquiring on the third rather than silently choosing another candidate. The faster `VNTrackObjectRequest` path reconciles display geometry but cannot clear detector misses or count as fresh composition evidence. It now uses stricter confidence/jump/scale gates plus position/size dead zones. Full detections reseed the sequence tracker only after material drift instead of every sample. Once locked, automatic Symmetry, Leading Lines, Negative Space, Lead Room, and ambiguity changes cannot clear or rebuild the selected policy/target anchor/target size; only an explicit photographer action or lifecycle reset can change the plan. This is a stable 2D subject-relative lock, not an ARKit world anchor. No identity/re-identification, ARKit session, Xiaoyi/provider call, Camera route/upload, auto capture, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P22-R1 fixes the Mac/Xcode diagnostics reported after P22. `LocalAIComposeTemporalSubjectTracker.translated(...)` now explicitly returns its normalized rectangle after computing bounded X/Y limits, removing the build-blocking missing-return/unused-initializer pair. `CameraCaptureService` imports AVFoundation through `@preconcurrency`, acknowledging the SDK's incomplete legacy `Sendable` annotations while preserving the existing dedicated serial session queue for start/stop. No camera lifecycle, preview, capture, AR/depth/filter behavior, provider boundary, or production setting changed. The separate Xcode "Update to recommended settings" project notice remains an operator-review item and was not applied blindly.
- Local AI Compose P22 adds foreground-aware AR guide occlusion on top of P20 synchronized hardware depth. Only reliable high-separation absolute depth may be reduced to a fixed 18×24 binary cell mask near the current Vision subject; the mask contains no depth values, confidence, identity, pixels, or semantic labels. Isolated cells are removed, enclosed holes are filled, two compatible full-analysis masks are required to activate, one missing sample is tolerated, and a distant replacement requires two samples. P17 motion entry and P21 thermal protection clear the spatial mask. The stabilized mask is mirrored only at front-camera display time and used inside an isolated SwiftUI compositing group with `destinationOut`, so composition grid/tint disappears behind the foreground while subject/target frames remain above it. Group Balance and unsupported/weak depth keep the ordinary overlay. No mask/depth persistence, logging, upload, analytics, training, ARKit, Xiaoyi/provider call, Camera route, automatic control/capture, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P21 adds a capture-first adaptive workload scheduler. Nominal power/thermal state keeps approximately 2 FPS full analysis and 12 FPS explicit-lock tracking; Low Power Mode or fair thermal pressure reduces them to approximately 1.18 FPS and 8 FPS. Serious/critical thermal pressure immediately stops Vision and fast tracking, emits only a coarse `thermallyPaused` state, clears action/pose/Ready evidence, removes live depth, dims preserved geometry, and shows a localized orange cooling reticle while preview, live filter, manual controls, and shutter stay available. Recovery requires five seconds outside serious/critical before analysis resumes, preventing thermal oscillation. Raw thermal readings/timestamps never cross into UI, logs, persistence, analytics, uploads, or scores. No ARKit, Xiaoyi/provider call, Camera route, auto-trigger, automatic camera control/capture, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P20 activates hardware depth only on compatible AVFoundation camera formats. `AVCaptureDepthDataOutput` is time-matched with the existing video output through `AVCaptureDataOutputSynchronizer`; unsupported lenses and dropped depth frames continue on the existing RGB/Vision path. A private sparse analyzer converts synchronized absolute `DepthFloat32` values into only near/mid/far, foreground/background separation, and confidence buckets. Two reliable strong-separation full-analysis samples activate a subtle on-device depth-layer halo, one clear sample is tolerated, and the second removes it. P17 device motion freezes this evidence, while Group Balance suppresses the single-subject depth cue. Raw depth maps, metric values, sample ratios, and subject/background medians never leave the analyzer and are not logged, persisted, uploaded, displayed, or used for training. No ARKit world session or Xiaoyi call is added because synchronized AVFoundation depth is sufficient; no Camera cloud route, direct iOS provider call/key, automatic capture, model artifact, or production rollout is added, and `productionReady:false` remains locked.
- Local AI Compose P19 adds a photographer-selected Group Balance mode for scenes with two to four plausible local subjects. It unions only the same bounded candidate set already shown by P11, requires two geometrically compatible full-analysis samples before activation, smooths compatible continuation, requires two consistent samples before replacing a distant group, tolerates one missing sample for display continuity, and clears on the second. A retained or pending group is explicitly not fresh action/Hold/Ready evidence. P17 device-motion pause freezes the controller and P13 fast tracking cannot update it. Selecting Group Balance clears any individual lock, draws one dashed cyan union frame with a group badge, uses a centered bounded target, and renders a distinct central safe-region grid; long-pressing an individual returns to automatic single-subject guidance. The app never automatically decides that visible people/objects belong together and never infers identity, relationship, importance, or quality. No new Vision request, ARKit, Xiaoyi/provider call, Camera upload/route, box/group history logging or persistence, automatic camera actuation/capture, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P18 adds a Lead Room composition family for an explicitly locked subject with stable horizontal screen-space motion. It reuses the existing full-analysis temporal tracker's ephemeral velocity and accepts only a fresh matched detector sample while the P17 device-motion gate is not paused. Two matching samples above a private entry threshold activate or reverse the left/right bucket; a lower continuation threshold prevents flicker, one missing/weak sample is tolerated, and the second clears it. Automatic policy keeps precise Leading Lines first, then bounded Lead Room, quiet-space Negative Space, Symmetry, and prior geometry fallback. A right-moving subject is guided toward the left third (and vice versa), while the overlay softly marks the open side with dashed motion lanes; manual policy and horizontal flip remain available. This is coarse geometry, not speed, gaze, intent, identity, activity, or aesthetic quality. P13 fast tracking only redraws and never advances the state. No new Vision request, ARKit, Xiaoyi/provider call, Camera upload/route, velocity/trajectory logging or persistence, automatic camera actuation/capture, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P17 adds a motion-coherent AR guidance gate using the existing short-lived CoreMotion monitor. A 500 ms local window combines average user-acceleration magnitude with gyroscope rotation-rate magnitude, then exports only `stable`, `moving`, or `unavailable`; raw sensor values remain inside the in-memory monitor. A moving bucket pauses new target creation plus horizon, scene-policy, ambiguity, pose, action, Hold, and Ready evidence on the next full-analysis sample. An existing target/subject outline may keep redrawing without advancing evidence. Two fresh stable/unavailable full-analysis samples release the pause, preventing one transient sample from immediately restoring guidance and preventing unavailable hardware from becoming permanently blocked. The overlay dims preserved composition geometry, hides movement/level cues, and shows a centered gyroscope-style 2D AR reticle with non-judgmental copy that still allows intentional handheld motion. No new Vision request, ARKit, Xiaoyi/provider call, Camera upload/route, raw sensor logging/persistence, automatic camera actuation/capture, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P16 makes Negative Space scene-aware without adding semantic cloud analysis. The existing P15 read-only Y-plane pass now also compares fixed left/right activity zones on the same 24×32 logical portrait grid and under the same Core Video lock. Private side activity, absolute difference, and quiet-to-active ratio reduce to `observed`, `notObserved`, or `unavailable` plus one left/right bucket; no measurement leaves the analyzer. Two matching fresh full-analysis samples activate or switch the side, and two clear samples remove it. A bounded small subject may receive a Negative Space target opposite the stable quieter side; leading lines retain priority, manual policy/target flip remain available, and the AR overlay softly marks the region to preserve. This is local luma activity context rather than a claim that an area is empty, uncluttered, or aesthetically better. No new Vision request or pixel-buffer lock, raw grid/activity/stat storage or logging, semantic/sensitive/identity inference, ARKit, Xiaoyi call, upload, automatic camera actuation, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P15 adds local leading-line convergence evidence for corridors, roads, rails, and other perspective structures. Only while Compose full analysis is active, a read-only Y-plane pass samples a fixed 24×32 logical portrait grid, retains at most four strong gradients in each 6×8 region, and bounds pairwise line intersections to at most 192 edges / 18,336 pairs. Private intersection weight, radial support, sector, and orientation-family checks reduce to `observed`, `notObserved`, or `unavailable` plus one bounded normalized convergence point; no numeric score crosses actors. Two nearby fresh observations activate a frozen target, two consistent distant observations replace it, and two clear samples remove it. Automatic Leading Lines is eligible only when a bounded subject is near that point; it takes precedence over weaker symmetry evidence, remains manually selectable, and renders dashed 2D AR guides toward the target. No contour/new Vision request, raw grid/edge/intersection/stat storage or logging, semantic/sensitive/identity inference, ARKit, Xiaoyi call, upload, automatic camera actuation, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P14 adds local scene-structure evidence so automatic policy is no longer derived from the subject rectangle alone. Only while Compose full analysis is active, an orientation-aware luma analyzer reads the existing full-range bi-planar Y plane under a read-only lock and samples 14×18 left/right pairs outside a narrow center strip. It reduces mirror difference, left/right balance, dynamic range, and local texture to `observed`, `notObserved`, or `unavailable`; no numeric score leaves the background call. Two fresh full-analysis samples are required to enter or leave symmetry, and an automatic state change rebuilds target/action/readiness without clearing an explicit subject lock. A stable structured scene plus a bounded near-center subject may select the new Symmetry policy; users may also choose it directly. Its AR grid draws a cyan center axis and paired quarter guides, and target flip remains available only for thirds/negative-space policies. No new Vision request, raw pixel/stat storage/logging, aesthetic score, identity/sensitive inference, ARKit, Xiaoyi call, upload, automatic camera actuation, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P13 smooths an explicitly locked AR subject between the existing approximately 2 FPS full-scene detections. A local `VNSequenceRequestHandler` runs one `.fast` `VNTrackObjectRequest` on the existing background frame queue at a 12 FPS ceiling and is seeded only after the photographer long-presses a candidate. Accepted detector matches issue fresh ephemeral seed tokens about every 0.5 seconds to correct drift; stale-token and nonmonotonic same-seed callbacks are rejected. Confidence/size/jump gates stay private, two misses are required before the UI returns to reacquiring, and a bounded display interpolation reduces rectangle jitter. Sequence updates redraw geometry only and never advance ambiguity, action, pose-edge, Hold, or Ready evidence. No identity recognition, raw-frame retention, token/box/confidence logging or persistence, extra detector request, ARKit, Xiaoyi call, upload, automatic focus/capture, model/training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P12 adds explicit local tap-to-focus and tap-to-expose without letting AI silently control the lens. Taps are accepted only inside the actual portrait 3:4 aspect-fit camera content; `AVCaptureVideoPreviewLayer` converts the visible layer point to the capture device coordinate system, including preview rotation, mirroring, and video gravity. The active camera is capability-checked and configured under a short AVFoundation lock, with auto modes preferred and continuous modes used only as supported fallbacks. A temporary orange focus reticle and local haptic acknowledge success; unsupported hardware gets a temporary dashed neutral reticle and warning haptic. VoiceOver has an explicit center-focus action. No tap/point history, automatic AI focus/exposure, frame upload, ARKit, Xiaoyi call, persistence, score, automatic capture, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P11 turns confirmed multi-subject ambiguity into an actionable AR selection state. After the existing two-fresh-frame ambiguity gate, up to four plausible candidates above the existing area floor are carried as geometry-only boxes and rendered as dashed cyan frames with small accent corner markers. Front-camera display uses the same mirrored coordinate mapper as target/subject overlays. During confirmed ambiguity, long-press hit testing is limited to exactly the highlighted candidate set; ordinary single-subject long-press behavior remains unchanged. Locking one candidate removes the alternatives and resumes the existing tracker/target loop. No candidate number, kind label, identity, confidence, score, persistence, new Vision request, ARKit, Xiaoyi call, upload, automatic capture, training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P10 makes the local composition policy user-steerable instead of treating one heuristic as authoritative. While Compose is active, a compact control offers AI choice, rule of thirds, centered balance, and negative space. For an active non-centered subject, the same menu can flip the target to the opposite horizontal side. A policy/side change rebuilds target geometry and resets action/Hold/Ready evidence while preserving the current subject lock/tracker; the preference remains in memory for the active Compose session and resets to AI choice when Compose is toggled. The UI exposes no aesthetic score or claim that one layout is best. No new Vision request, ARKit, Xiaoyi call, upload, persistence, automatic camera control/capture, training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P9 completes the manual closed loop with a truthful hold-to-ready state. Temporal ambiguity, pose-edge, action-switch, and readiness controllers now advance only from fresh analyzed frames; toggles, long presses, target resets, and ordinary UI refreshes cannot counterfeit repeated evidence. When the stable action first reaches alignment, the overlay asks the user to hold and shows an accent scope/dashed target ring. A second fresh aligned frame changes the target to mint Ready and emits one success haptic on that transition only. Any non-aligned evidence resets readiness; there is no score, percentage, countdown, shutter gate, or automatic capture. No new Vision request, ARKit, Xiaoyi call, upload, persistence, training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P8 hardens the preview-to-Vision orientation contract. `AVCaptureVideoDataOutput` keeps its hardware 90-degree portrait rotation for the live filter path; when that rotation is applied, Vision now receives `.up` instead of interpreting the already-rotated pixel buffer as `.right` a second time. If rotation is unavailable, the contract falls back to sensor-native landscape plus Vision `.right`. Preview rotation, Vision orientation, filtered-preview fallback, Compose overlay mapping, and long-press selection now share one typed contract and one portrait 3:4 source size. This adds no dynamic interface-orientation rotation, extra Vision request, ARKit, Xiaoyi call, upload, persistence, automatic camera control/capture, or production rollout; `productionReady:false` remains locked.
- Local AI Compose P7 adds a conservative pose-edge framing guard to the P6 AR action loop. The already-running `VNDetectHumanBodyPoseRequest` now reduces high-confidence body points to a four-edge bitmask when at least five reliable points place a pose within 4.5% of the preview boundary; raw joints and confidence values stay inside the background analyzer. Two matching samples are required to activate or change a warning and two clear samples remove it. Once normal X/Y placement is close, stable edge evidence selects the existing leave-space action and renders a subtle orange marker on the affected display edge, including left/right mirroring for the front camera. It adds no extra Vision request, skeleton, score, persistence, automatic camera control/capture, ARKit, Xiaoyi call, Camera upload/route, training, or production rollout; `productionReady:false` remains locked.
- Local AI Compose P6 turns the P5 coach from text-led guidance into a stabilized AR action layer. A typed local resolver now compares horizontal and vertical displacement relative to their own tolerances and chooses the dominant placement error before scale, rather than always prioritizing horizontal movement. The first action appears immediately; subsequent left/right/up/down/zoom/space/aligned switches require two agreeing samples, with action-specific exit hysteresis around movement, scale, and aligned thresholds. The overlay draws a bounded cyan/accent arrow in the stable screen-space direction and uses nonnumeric zoom/step-back/aligned center cues. Front-camera target mapping remains mirrored, visual cues are decorative for accessibility, and all existing grids, horizon, subject lock, and capture gestures remain active. No score, raw action history logging/persistence, automatic camera control/capture, ARKit, Xiaoyi, Camera upload/route, training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P5 adds optional image-content horizon intelligence to the P4 AR-style guide. Only while Compose is active, the existing approximately 2 FPS background Vision pass may run `VNDetectHorizonRequest`; observations below a confidence gate or beyond a bounded ±25-degree range are discarded, and only half-degree rounded angles cross to the main actor. Two agreeing samples activate a smoothed scene-horizon cue, one miss is tolerated, and the second clears it. The overlay compares a cyan/mint detected-horizon line with a dashed horizontal reference, mirrors its sign for the front preview, and keeps straightening optional so deliberate tilt remains valid. Device CoreMotion level stays the fallback when no stable scene horizon exists. No raw observation/confidence/frame logging or persistence, ARKit world session, Xiaoyi call, Camera upload/route, score, automatic correction/capture, training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P4 adds a stable on-device composition-policy layer above the P3 subject tracker. Ephemeral geometry can now select `thirds`, `centered`, or `negativeSpace` with bounded, explainable rules for subject kind, area, aspect, and location; the chosen policy and target remain session-stable. Multiple plausible subjects must persist for two Vision samples before the guide pauses and asks for a long-press selection, and two clear samples are required before automatic guidance resumes. The overlay shows the selected policy and renders a matching thirds/dashed-negative-space or centered AR-style grid. No aesthetic score, sensitive/identity inference, ARKit world session, Xiaoyi call, Camera upload/route, persistence, training, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P3 replaces the P2 nearest-center continuation with a geometry-only temporal matcher. The selected candidate kind is carried forward using a short motion prediction, box intersection-over-union, size consistency, bounded reacquisition distance, and visual box smoothing. When two viable candidates have nearly equal scores, the matcher rejects the frame as ambiguous and keeps the locked guide in its reacquiring state instead of guessing. This remains ephemeral on-device geometry, not identity recognition or face tracking; it adds no ARKit world session, Xiaoyi call, Camera upload/route, model artifact, persistence, or production rollout, and `productionReady:false` remains locked.
- Local AI Compose P2 adds an explicit long-press subject lock and AR-style 2D alignment layer to the P1 local composition loop. Vision now returns at most six ephemeral face/body or generic objectness candidates; a long press selects the containing or nearest bounded candidate, locks the target position/scale, and follows only the same candidate kind near the prior center. A temporarily lost subject enters a safe reacquiring state instead of jumping to another subject. Front-camera touch coordinates are unmirrored before local matching, and the locked subject gets a cyan AR-style frame plus haptic success/warning feedback. No ARKit world tracking is needed for this 2D preview overlay, and Xiaoyi `gpt-5.6-luna` is not called because subject selection and alignment are lower-latency and safer on-device. No preview upload, Camera provider route, model artifact, persistence, or production rollout is added; `productionReady:false` remains locked.
- Local AI Compose P1 adds an explicit Camera `AI Compose` mode inspired by the behavior shown in the supplied Doka Cam Reel, without copying its brand/assets/code. The existing 2 FPS local Vision path now uses face/body geometry first and objectness saliency only as a generic-subject fallback. A session-stable rule-of-thirds target drives a target frame, current-subject outline, directional placement hint, distance/zoom hint, optional device-level line, and aligned/hold state. Preview frames, subject rectangles, saliency results, and Compose outputs remain in memory only and are not uploaded, logged, or persisted. This is a bounded deterministic prototype, not a custom Doka-equivalent trained model; governed dataset/label/license/consent/human-review work remains a separate later phase. Camera has no cloud AI entry, and `productionReady:false` remains locked.
- PT2-SF-R9-R16-R2 is an experimental polished candidate for transferable Recipe v2 tone. The model prompt gives `lumaCurve` primary reusable-tone ownership and keeps exposure/contrast/fade/shadow controls residual. Before film texture/glow/vignette, iOS compares alpha-aware 64-pixel source/tone-color samples over a 6×6 grid. Correction requires repeated opaque, low-chroma, textured-shadow evidence and uses a local chromaticity-preserving 17-cube plus a bounded piecewise mask; declared low-key curves, saturated shadow styling, vignette, and Recipe `1.1` stay outside the guard. A 100 ms intensity debounce reduces queued preview work. The selected style/reference image still follows the existing consented backend path; the apply/original image and all adaptive samples/statistics remain local and unpersisted. `productionReady:false` stays locked, while Mac/Core Image multi-style validation remains pending.
- PT2-SF-R9-R16-R1 corrects the compound black-floor lift visible in the operator-confirmed 100% Recipe v2 result. The backend contract and iOS validator now give black/white endpoint ownership to the luma curve, require exact `0/1` endpoints on the R/G/B channel curves, and enforce the same deterministic budget across luma black point, fade, shadow lift, and negative contrast. The saved real recipe regression is normalized from `contrast:-0.03`, `fade:0.07`, and `shadowLift:0.05` to `contrast:0`, `fade:0.048837`, and `shadowLift:0.034884`, while preserving its warm colour intent. The backend still receives one style/reference image only; apply/original and rendering stay local, and `productionReady:false` remains locked.
- PT2-SF-R9-R16 upgrades Filter Lab to strict recipe `2.0`. The original 12 global controls remain as bounded fine adjustments, while the local renderer gains safe five-point luma/R/G/B curves, a deterministic 17-cube built from eight app-owned basis looks, luminance normalization, grain size/roughness/luma response, warm highlight halation, and diffusion. Historical `1.1` recipes decode with identity v2 values. User intensity now blends the complete filtered result with the untouched local original, so 0/50/100% have consistent meaning. A bounded target-only Luna check accepted recipe `2.0` in one attempt and wrote only a v2 sanitized artifact. The backend still receives one style/reference image only; the apply/original image and all colour-cube rendering remain local. No arbitrary LUT/shader/code, iOS provider key/direct call, Camera cloud entry, or production rollout is added, and `productionReady:false` remains locked.
- PT2-SF-R9-R15-X1 rebuilds the Xiaoyi path for `gpt-5.6-luna` from the supplied integration guide and OpenAPI records. Runtime mode `xiaoyiLunaInternal` resolves a new `XiaoyiLunaRelayProvider`, not the historical DeepSeek adapter. A replacement-key text smoke passed with HTTP 200, `finish_reason:stop`, and the exact expected reply. One synthetic-image Filter Lab request also returned HTTP 200, but the generated output failed recipe `1.1` validation in sanitized bucket `provider_invalid_schema`; image transport is accepted but Luna visual/recipe compatibility is not yet approved. The running backend remains on the working SiliconFlow rollback and `productionReady:false` remains locked.
- PT2-SF-R9-R15 improves Filter Lab fidelity and adds an opt-in, ignored server analysis artifact containing only the normalized validated recipe plus safe metadata buckets. Final rendered pixels now outrank unknown third-party preset/slider values; recipe `1.1` adds shadow lift, highlight roll-off, bloom, and dust, while the local renderer lifts blacks, separates grain from analog defects, and uses a gentler vignette. Raw images/base64, prompts, requests, provider responses, Authorization headers, and keys are never persisted. No provider call was made in this phase; Mac/Xcode rendering and a separately approved provider comparison are pending. The backend still receives only one style/reference image, the apply/original stays local, Camera remains local-only, and `productionReady:false` remains locked.
- PT2-SF-R9-R14 adds a user-initiated local export for the Filter Lab filtered preview. Pressing and holding the after image saves the currently rendered 1600-pixel-bounded preview to Apple Photos using add-only authorization, with localized progress/success/failure feedback and a VoiceOver action. It does not save automatically, call the backend/provider, change the one-style-image upload contract, upload the apply/original image, add Camera cloud AI, or enable production rollout; `productionReady:false` remains locked. Mac/Xcode physical-device permission and saved-asset verification are pending.
- PT2-SF-R9-R13 calibrates Filter Lab recipe fidelity from prompt through local rendering. Backend Filter Lab now requests a strict structured recipe, separates photographed content from transferable style, and gives the vision model the exact Core Image control semantics; iOS renders exposure once, applies bounded local monochrome grain, and previews generated recipes at full strength by default. A sanitized no-network fidelity evaluator and regression tests were added. No provider call is made in this phase, the backend still receives exactly one style/reference image, the apply/original image stays local, iOS gains no provider key/direct provider URL, Camera gains no cloud AI entry, and `productionReady:false` remains locked. Mac/Xcode physical-device visual and memory checks plus a separately approved provider compatibility run are still pending.
- PT2-SF-R9-R12 fixes the physical-device Filter Lab memory pressure found after Cloud AI successfully connected and generated a recipe. Filter Lab now downsamples imported PhotosPicker images before storing them in the view model, caps generated preview rendering to a bounded long edge, disables Core Image intermediate caching for that preview path, and clears CI caches after render. It adds no provider key/direct provider URL to iOS, no backend payload expansion, no apply/original image upload, no Camera cloud AI entry, no production rollout, and keeps `productionReady:false` locked.
- PT2-SF-R9-R11 fixes the misleading Filter Lab Cloud debug readiness signal. Backend `/health` now returns safe provider readiness buckets instead of hardcoded `mock-only`, and the DEBUG Filter Lab endpoint Test decodes `filterLabReady` so the physical iPhone can show whether the LAN backend is reachable and truly ready for Filter Lab Cloud. It runs no provider smoke, adds no provider key/direct provider URL to iOS, expands no backend payload, uploads no apply/original image, adds no Camera cloud AI entry, makes no production rollout, and keeps `productionReady:false` locked.
- PT2-SF-R9-R10 fixes the remaining physical-device Filter Lab Cloud connection path by adding a DEBUG in-app backend URL/health panel, normalizing the app backend base URL to localhost/private LAN only, building backend route URLs without encoded slash risk, requiring DEBUG Filter Lab to receive a real `cloud` recipe before showing a result, and adding DEBUG local-network ATS allowance for the LAN HTTP backend. It adds no iOS provider key/direct provider URL, no provider SDK, no backend payload expansion, no apply/original image upload, no Camera cloud AI entry, no production rollout, and keeps `productionReady:false` locked. Next recommended step is an Xcode physical-device retry: in Filter Lab enter `http://192.168.68.60:8787`, tap Save, tap Test, then run the DEBUG Filter Lab backend test.
- PT2-SF-R9-R9 fixes the Windows local backend startup/LAN reachability blocker for physical-device Filter Lab Cloud testing. `npm start` now correctly starts `backend/src/server.mjs`, loads ignored server-side local env files without printing secrets, and the local backend has been verified reachable at both `http://127.0.0.1:8787/health` and `http://192.168.68.60:8787/health`. It runs no provider smoke, adds no iOS provider key/direct provider URL, expands no payload, changes no Swift/Xcode project files, adds no Camera cloud AI entry, makes no production rollout, and keeps `productionReady:false` locked. Next recommended step is the operator physical-device Filter Lab retry against `AI_PHOTO_CLOUD_AI_BASE_URL=http://192.168.68.60:8787`.
- PT2-SF-R9-R1 fixes physical-device LAN debug launch/setup by adding generated Info.plist local network usage text and DEBUG local networking allowance for the phone-to-computer backend path. It adds no provider key/direct provider URL, no provider SDK, no backend runtime change, no provider smoke, no payload expansion, no Camera cloud AI entry, no production rollout, and keeps `productionReady:false` locked.
- PT2-SF-R9 adds a DEBUG-only iOS backend base URL override for physical-device Filter Lab testing. Xcode DEBUG schemes can set `AI_PHOTO_CLOUD_AI_BASE_URL=http://<computer-lan-ip>:8787` so the iPhone reaches the local/LAN backend while provider credentials remain server-side only. It adds no iOS provider key/direct provider URL, no provider SDK, no Xcode project change, no backend runtime change, no provider smoke, no apply/original image upload, no Camera cloud AI entry, no production rollout, and keeps `productionReady:false` locked. Next recommended step is `PT2-SF-R9-RUN`, an operator physical-device LAN backend verification.
- PT2-SF-R8-R2 ran one fresh-approved DEBUG Filter Lab backend app endpoint smoke after the ignored server-side debug gate was corrected. `/v1/ai/filter-lab` returned `source:cloud` with a generated filter present from one style reference image, no apply/original image upload, no direct iOS provider call, no Swift/backend runtime/Xcode payload change, no provider credential commit, and `productionReady:false` locked. Next recommended step is `PT2-SF-R9`, a physical-device DEBUG Filter Lab backend E2E smoke over a phone-reachable local/LAN backend while keeping provider keys server-side only.
- PT2-SF-R7 updates the iOS Filter Lab flow to use two image slots: one target filter reference image for generated recipe analysis and one original/apply image for local before/after preview rendering. The backend Filter Lab request remains one reference image only; the apply image stays local, the Camera remains local-only, no iOS provider key/direct provider URL is added, no backend payload expansion or production rollout is added, and `productionReady:false` remains locked.
- PT2-SF-R6 wires the backend internal/debug app endpoints to the active SiliconFlow provider mode. `/v1/ai/photo-advisor` and `/v1/ai/filter-lab` can now route through server-side-only `siliconflowInternal` config using `Qwen/Qwen3-VL-32B-Instruct`, while keeping iOS provider keys/direct provider URLs blocked, Camera local-only, production/default mock/local, no provider credential commit, no live credential smoke in this phase, and `productionReady:false` locked.
- PT2-SF-R3 adds the backend-only SiliconFlow Filter Lab structured recipe QA gate. The gate defaults to dry-run/no-network/no-image-read and requires explicit `--run-provider`; the approved one-image synthetic provider QA with `Qwen/Qwen3-VL-32B-Instruct` made exactly one provider/model call, read exactly one ignored synthetic image, attempted exactly one image upload, returned `acceptedCount:1`, `httpStatusBucket:2xx`, and latency bucket `gt_15s`. The output contract is validated generated filter recipe JSON only, with generated bitmap output, shader/code, LUT URL, rendering instruction, raw recipe/provider text, iOS runtime, Camera cloud AI entry, upload payload change, image editor provider, StoreKit/payment, and production rollout all blocked. `productionReady:false` remains locked.
- PT2-SF-R2C runs the explicitly approved backend-only SiliconFlow one-image Photo Advisor QA with `Qwen/Qwen3-VL-32B-Instruct`. The sanitized synthetic run made exactly one provider/model call, read exactly one ignored synthetic image, attempted exactly one image upload, returned `acceptedCount:1`, `httpStatusBucket:2xx`, and latency bucket `5s_to_15s`. It commits no provider credential, raw prompt/payload/provider response/image/base64, iOS runtime, Camera cloud AI entry, upload payload change, image editor provider, StoreKit/payment, or production rollout.
- PT2-SF-R2B adds a backend-only, no-network SiliconFlow vision-capable model preflight gate for Photo Advisor image QA. It selects `Qwen/Qwen3-VL-32B-Instruct` as the current recommended one-image QA candidate, keeps `deepseek-ai/DeepSeek-V4-Flash` as text-only smoke evidence but blocked for image QA, and updates the existing image QA gate to accept explicit `--model-candidate=qwen3_vl_32b_instruct`. It reads no API key, opens no image, makes no provider/model call, adds no iOS runtime, Camera cloud AI entry, upload payload change, provider credential commit, raw prompt/payload/provider/image logging, image editor provider, StoreKit/payment, or production rollout.
- PT2-SF-R2 adds a backend-only SiliconFlow Photo Advisor image QA gate for ignored synthetic / approved-real samples. The gate defaults to dry-run/no-network/no-image-read and requires explicit `--run-provider`; a bounded one-image synthetic run reached SiliconFlow but `deepseek-ai/DeepSeek-V4-Flash` returned sanitized `provider_vision_request_rejected` / `4xx`, so the next recommended step is selecting or preflighting a SiliconFlow vision-capable model before Filter Lab or iOS integration. It adds no Swift runtime, iOS project setting change, Camera cloud AI entry, upload payload change, provider credential commit, raw prompt/payload/provider/image logging, image editor provider, StoreKit/payment, or production rollout.
- Phase 21-A7-VERIFY adds a docs-only physical-device QA handoff and roadmap-based inventory of features not yet installed in the app runtime. It does not claim Xcode/iPhone verification from Windows and adds no Swift runtime, iOS project setting change, cloud AI, provider/model call, provider key, preview-frame upload, upload payload change, raw frame persistence, model package, dataset crawler, AI-assisted labeling run, user-photo training, or production rollout.
- Product correction: Camera-page Live Cloud AI / Live Advisor is no longer needed because local on-device guidance is the Camera path. Future Cloud AI should be scoped to Inspiration / post-capture photo analysis, generated filters, and image editing through backend-mediated mainland relay providers with server-side-only keys.
- Phase 21-A7 polishes the non-composition Camera guidance runtime after A6. Local guidance now falls back to the safe local-unavailable hint when local signals produce no visible post-A6 suggestion, and compact local guidance uses a plain guidance icon instead of a suggestion-like sparkle icon when no hint is visible. It adds no cloud AI, provider/model call, provider key, preview-frame upload, upload payload change, raw frame persistence, dataset crawler, AI-assisted labeling run, user-photo training, Depth Anything/Core ML model runtime, Florence runtime, or production rollout.
- Phase 21-A6 aligns the iOS Camera runtime with the Phase 21-A5 ownership boundary. Local Vision / AVFoundation geometry signals may still be collected in memory, but app-authored composition and portrait-layout hint copy is suppressed so richer composition guidance remains owned by the separate training-AI branch. Lighting/filter/unavailable local hints remain available. It adds no cloud AI, provider/model call, provider key, preview-frame upload, upload payload change, raw frame persistence, dataset crawler, AI-assisted labeling run, user-photo training, Depth Anything/Core ML model runtime, Florence runtime, or production rollout.
- Phase 21-A5 records the composition guidance ownership boundary: richer composition advice should be owned by the training-AI branch, while the app-side Camera runtime remains a lightweight local signal / hint-shell layer. It adds no Swift runtime, cloud AI, provider/model call, provider key, preview-frame upload, upload payload change, dataset crawler, AI-assisted labeling run, user-photo training, raw frame persistence, Depth Anything/Core ML model runtime, Florence runtime, or production rollout.
- Phase 21-A4 improves the local-only live framing hint UX on top of the existing Apple Vision / AVFoundation signal path. Expanded local guidance now uses a user-facing `Next hint` action instead of debug-style state cycling, visible local hints can be manually rotated, and compact/expanded hints use category-specific icons. It adds no cloud AI, provider/model call, provider key, preview-frame upload, upload payload change, raw frame persistence beyond in-memory rendering, Depth Anything/Core ML model runtime, Florence runtime, or production rollout.
...
- Recommended practical next step depends on direction: for app-side feature development, choose a non-composition Camera feature or focused UI/runtime polish; for composition intelligence, continue in the training-AI branch with dataset/label schema and legal/source/consent gates before any AI-assisted labeling or model work. Phase 21-C-R3-RUN remains available only after the exact approval phrase in `docs/phase-21-c-r3-depth-anything-operator-artifact-verification-and-physical-device-benchmark-approval-request.md` is provided, a local ignored artifact is ready, and MacBook/Xcode physical-device benchmarking is available. Any model artifact handling, Core ML package use, inference execution, benchmark run, Camera runtime integration beyond explicit local scope, upload path, provider/cloud call, iOS provider/model key, raw artifact, sensitive inference, raw frame persistence, dataset crawler, user-photo training, or production rollout requires separate explicit approval.
...
PT2-SF-R9-R15-X1 adds a fresh Xiaoyi Luna backend path under `xiaoyiLunaInternal`, separate from the historical DeepSeek relay adapter. Sanitized field diagnostics aligned the Luna prompt without weakening the strict validator; zero-retry synthetic-image, backend route, and running HTTP endpoint checks now return validated recipe `1.1` with all 12 renderer parameters. The current internal/debug backend process runs Luna while SiliconFlow remains the rollback configuration. Filter Lab returns only a validated structured recipe, never a generated bitmap, shader/code, LUT URL, or rendering instruction. The key remains external/server-side, iOS has no provider SDK/key/direct provider call, production/default behavior remains mock/local, Camera remains local-only, and `productionReady:false` remains locked.

PT2-SF-R9-R15-X2 corrects the first real-reference Luna result, which washed the original into mid-gray. Sanitized comparison showed the generated q10 black level at `0.506` versus target `0.214`; the saved recipe compounded negative contrast, fade, and shadow lift, while the iOS renderer amplified shadow lift at `1.8x`. The backend prompt now excludes screenshot UI from photographic estimates, preserves deep photographic blacks, bounds the combined faded-look guidance, and requests high-detail Filter Lab vision. The iOS renderer maps shadow lift at a bounded `0.55x`. A two-call, zero-retry real-reference QA ended with a validated recipe using `contrast:0.01`, `fade:0.06`, and `shadowLift:0.04`; full tests pass, and physical-iPhone visual retest is pending. Boundaries and `productionReady:false` remain unchanged.
...
PT2 is still not a production rollout. It now uses backend-only SiliconFlow OpenAI-compatible internal/debug credential smoke and QA gates for Photo Advisor and Filter Lab with server-side secrets only. Text-only smoke accepted for `deepseek-ai/DeepSeek-V4-Flash`, PT2-SF-R2 image QA is blocked by `provider_vision_request_rejected` for that model/request path, PT2-SF-R2B no-network preflight recommended `Qwen/Qwen3-VL-32B-Instruct`, and PT2-SF-R2C accepted one sanitized synthetic Photo Advisor image QA call with that candidate. No iOS provider key/direct provider call, Camera cloud AI entry, image editing provider, StoreKit/payment, or production rollout is added.
