...
- PT2-SF-R8-RUN ran the approved DEBUG Filter Lab backend smoke exactly once. It sent one in-memory style reference image only, sent no apply/original image, printed no raw payload/base64/provider response/key/header, and returned sanitized `2xx` fallback `internal_cloud_disabled` before any provider/model call because the ignored server env gate was still closed (`CLOUD_AI_PROVIDER_MODE` resolved to `mock`, `ALLOW_INTERNAL_CLOUD_AI` resolved to `false`). No Swift/backend runtime/Xcode payload change was made, no direct iOS provider call was added, and `productionReady:false` remains locked. Next recommended step is `PT2-SF-R8-R1` to correct only the ignored server-side env gate, then request fresh approval before a second smoke.
