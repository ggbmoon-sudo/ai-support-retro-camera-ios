...
PT2 now uses SiliconFlow as the active backend-only provider direction for internal/debug Photo Advisor and Filter Lab credential work, with `deepseek-ai/DeepSeek-V4-Flash` through the official OpenAI-compatible `/v1/chat/completions` path. PT2-SF-R1 adds a text-only SiliconFlow credential smoke gate that defaults to dry-run/no-network and requires explicit `--run-provider --surface=...` for provider execution. Filter Lab receives a validated structured recipe only in later QA phases, not generated bitmaps, shader/code, LUT URLs, or rendering instructions. iOS still has no provider SDK, provider key, or direct SiliconFlow call; production/default behavior remains mock/local, and Camera remains local-only.
...
PT2 is still not a production rollout. It now uses backend-only SiliconFlow OpenAI-compatible internal/debug credential smoke work for Photo Advisor and Filter Lab with `deepseek-ai/DeepSeek-V4-Flash`, server-side secrets only, and `imageUploadAttempted:false` for this phase. No iOS provider key/direct provider call, Camera cloud AI entry, image editing provider, StoreKit/payment, or production rollout is added.
