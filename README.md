...
PT2 now uses SiliconFlow as the active backend-only provider direction for internal/debug Photo Advisor and Filter Lab credential work. `deepseek-ai/DeepSeek-V4-Flash` remains the text-only credential-smoke model, but PT2-SF-R2 image QA found the current DeepSeek-V4-Flash image request blocked with sanitized `provider_vision_request_rejected`. PT2-SF-R2B selected `Qwen/Qwen3-VL-32B-Instruct`, and PT2-SF-R2C accepted one synthetic Photo Advisor image QA call with that candidate. Filter Lab receives a validated structured recipe only in later QA phases, not generated bitmaps, shader/code, LUT URLs, or rendering instructions. iOS still has no provider SDK, provider key, or direct SiliconFlow call; production/default behavior remains mock/local, and Camera remains local-only.
