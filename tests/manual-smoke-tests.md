...
- [ ] Confirm this commit is docs-only planning and adds no backend preflight gate/script/tests.
- [ ] Confirm `ios-app/AIPhotoApp/Features/Camera/DepthAnythingV2SmallSandbox.swift` remains untracked and is not staged.
- [ ] Confirm no Swift runtime file is included in this commit.
...
Backend manifest verification:

- [ ] From `backend/`, run `npm run qa:aesthetic-dataset:manifest`.
- [ ] From `backend/`, run `node --test tests/aesthetic-dataset-manifest-schema.test.mjs`.
...
- [ ] From `backend/`, run `npm run qa:aesthetic-parameters:tuning-dry-run-from-bridge`.
- [ ] From `backend/`, run `node --test tests/aesthetic-parameter-tuning-dry-run-from-bridge.test.mjs`.
- [ ] Confirm CLI output is sanitized and reports `runMode:"parameter_tuning_dry_run_from_bridge_no_export"` and `acceptedForTuningDryRunReview:true`.
- [ ] Confirm proposed parameter adjustments are review-only and each reports `candidateOnly:true`, `automaticMutationApplied:false`, `productionMutationAllowed:false`, `internalOnly:true`, and `userFacing:false`.
