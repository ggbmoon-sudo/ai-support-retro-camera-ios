...
- OD-R8F builds a review-only parameter-pack candidate scaffold from only those two accepted OD-R8E candidates. It excludes the `needs_more_data` candidates, emits internal parameter policy candidates only, and still does not export a parameter pack, read images, write reports, write to app runtime, or add user-facing scores/advice.
