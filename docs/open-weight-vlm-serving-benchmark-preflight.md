...
Phase 21-M adds `docs/quantization-serving-benchmark-plan.md` and `npm run qa:open-weight-vlm:quantization-serving-benchmark-plan`. Phase 21-W-R1 later clarifies the future target candidate as `Qwen3-VL-30B-A3B` for a separately approved model upgrade/benchmark phase. The plan keeps this preflight no-runtime boundary intact while recording Qwen2.5-VL reference, vLLM primary benchmark candidate, SGLang structured-output/performance challenger, Transformers+FastAPI reference, Ollama/LM Studio manual-only, and fp16/bf16, INT8, INT4, AWQ, GPTQ, or equivalent quantization classes.

Phase 21-M still does not run a serving benchmark, download weights, switch stacks, enable `local_model`, call vLLM/SGLang/Ollama, run Qwen inference, run fixture inference, add an endpoint, add an iOS runtime dependency, or approve production rollout. Phase 21-N was later explicitly approved for one backend local/private model call, but preflight blocked before execution because `smoke_001` was not present/approved in the ignored local fixture registry. The next recommended Phase 21-N-R0 resolves that local fixture prerequisite only.
