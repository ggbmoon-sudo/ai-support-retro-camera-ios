...
## Decision 035 - Bounded live-like Xiaoyi composition keyframes

Status: Accepted on 2026-07-20 by explicit user request for a 1 FPS Gemini-Live-like Camera experiment

Within the existing DEBUG/internal Xiaoyi Camera boundary, one explicit consent may start one live-like composition session. The app may submit independent metadata-stripped preview JPEG keyframes through the backend at an upper bound of one frame per second. There must be exactly one in-flight request, no frame queue, no catch-up burst, no background continuation, and no silent parallel retry. The next sample may be requested only after the previous request finishes and the cadence interval elapses.

Every `1.1` provider response must contain an exact enum-only composition plan, a safe subject kind, and a bounded subject rectangle grounded to the photographer's focus hint. Malformed, extra-field, free-form, scored, sensitive, identity-adjacent, or focus-inconsistent output fails closed. The first valid strategy may apply immediately; a changed strategy requires two matching later observations. Apple Vision carries the current subject locally at a nominal 15 FPS between cloud replies. A healthy local track takes precedence over stale cloud keyframe coordinates, while cloud grounding may recover missing local geometry.

This decision supersedes Decision 034's one-request limit only inside this explicit DEBUG/internal session. It does not approve true Gemini Live, bidirectional streaming, SSE as a frame-input channel, WebSocket/WebRTC, audio, queued video, release/default Camera cloud AI, automatic zoom/focus/exposure/lens/crop/shutter/capture, ARKit world anchoring, direct iOS provider access/key/URL/SDK, raw cloud artifact logging or persistence, analytics/training use, identity/sensitive inference, or production rollout. Stop/lifecycle/capture boundaries cancel the session and reject stale replies. `productionReady:false` remains locked.

## Decision 036 - Default local Compose with explicit Live AI consent

Status: Accepted on 2026-07-20 by explicit user request to make Live AI Compose the default Camera workflow
...
## Decision 040 - One Live AI entry with versioned remembered acceptance

Status: Accepted on 2026-07-22 by explicit user request to retain only Live AI composition

Camera must expose one DEBUG/internal Live AI start/stop control rather than a Local AI toggle plus a composition-policy menu. AI Choice, Rule of Thirds, Centered Balance, Symmetry, Leading Lines, Lead Room, Group Balance, Negative Space, and target-side flipping are no longer photographer-selectable UI. Their typed geometry policies remain internal implementation details because the validated cloud plan still needs a bounded local target and because all post-response Ring/frame tracking remains local.

The first start must still present the versioned image-processing disclosure before any upload. Accepting it may store one local boolean scoped to that exact disclosure version; later starts may construct the same accepted consent payload without showing the unchanged sheet again. Settings must provide a reset action, and any material disclosure/version change must use a new preference key and ask again. This remembered acceptance is not training consent, stores no image or provider artifact, and does not authorize background or repeated upload.

Local composition analysis defaults off and may activate only as an implementation detail of an accepted Live AI session. Stop and normal Camera context/lifecycle resets must not reveal a standalone local composition mode. Decision 040 does not change the one-keyframe/one-response contract, immutable plan, backend-only provider boundary, DEBUG-only entry, manual camera controls, absence of ARKit world anchoring, or `productionReady:false`.
