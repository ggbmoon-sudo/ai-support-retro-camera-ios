...
Reason: Phase 21-Z-R1 adds a docs-only API-first serverless VLM alternative gate before any RunPod provisioning or Qwen3 setup. SiliconFlow and DashScope / Alibaba Cloud Model Studio / 阿里雲百煉 are candidate providers only, and RunPod A100 80GB remains the self-hosted fallback/comparison path. The next step should verify account, pricing, terms, data-governance, Asia latency assumptions, rate limits, and structured-output viability without model calls, benchmarks, iOS runtime changes, upload payload changes, or production rollout.
...
Phase 21-Z2A should verify SiliconFlow and DashScope account feasibility, current pricing, exact VLM model availability, payment/currency/invoice requirements, provider terms, retention/training policy, Korea/Taiwan/Hong Kong latency expectations, rate limits, and structured JSON behavior before any API benchmark. If more provider-neutral comparison is needed first, use Phase 21-Z2: API vs RunPod Decision Gate Without Model Calls. RunPod remains available as Phase 21-Z1: Asia-first RunPod Instance Security + No-model Contract Gate, but it is no longer necessarily the next step if API-first is primary. Later branches may include Phase 21-Z2B API Provider No-model Contract Adapter Gate, Phase 21-Z2C Approved 12-fixture API VLM Benchmark, Phase 21-Z2D API vs RunPod Cost/Latency/Privacy Decision, and Phase 22-A debug-only iOS backend integration planning after provider benchmark review. Do not call SiliconFlow/DashScope APIs, create provider keys, provision RunPod, run another benchmark, run diagnostic model calls, rerun fixtures, call SGLang/vLLM/Ollama, run quantization benchmarks, run concurrency benchmarks, run Live Advisor simulation, download models, switch serving stacks, add app-facing endpoints, add iOS integration, or start production rollout without separate explicit approval.
