...
Reason: PT2-SF-R8-R1 corrected the ignored local backend debug env gate without running a second smoke. The no-network gate check now resolves Filter Lab to `siliconflowInternal` with the existing debug header, with no app endpoint request and no provider/model call. The next step is a fresh explicitly approved retry because the original R8-RUN already consumed its one allowed endpoint request. The retry must remain one DEBUG backend request only, one style reference image only, no apply/original image upload, no direct iOS provider call, no raw logging, and `productionReady:false`.
...
2. `PT2-SF-R2 - SiliconFlow Photo Advisor Internal Image QA`: implemented backend-only image QA gate; one synthetic provider image call reached SiliconFlow but current `deepseek-ai/DeepSeek-V4-Flash` returned sanitized `provider_vision_request_rejected`.
3. `PT2-SF-R2B - SiliconFlow Vision-capable Photo Advisor Model Selection and Preflight`: implemented no-network preflight; selected `Qwen/Qwen3-VL-32B-Instruct` as current one-image QA candidate, kept `Qwen/Qwen3-VL-8B-Instruct` as fallback canary, kept `Qwen/Qwen3-VL-30B-A3B-Instruct` as historical candidate requiring listing recheck, and blocked DeepSeek-V4-Flash for image QA based on R2 evidence.
4. `PT2-SF-R2C - SiliconFlow Qwen3-VL-32B One-image Photo Advisor QA`: implemented after explicit approval; one synthetic image provider QA with `--model-candidate=qwen3_vl_32b_instruct`, retry 0, sanitized aggregate only, accepted with `httpStatusBucket:2xx`, and no iOS/runtime/upload-payload changes.
5. `PT2-SF-R3 - SiliconFlow Filter Lab Structured Recipe QA`: implemented after explicit approval; one synthetic image provider QA with `--model-candidate=qwen3_vl_32b_instruct`, retry 0, sanitized aggregate only, accepted with `httpStatusBucket:2xx`, validated generated filter recipe JSON only, and no shader/LUT/bitmap/direct-render/iOS/runtime/upload-payload changes.
6. `PT2-SF-R4 - Debug-only iOS Inspiration Backend Integration Plan`: completed planning-only in `docs/pt2-sf-r4-debug-ios-inspiration-backend-integration-plan.md`; defines selected-photo Photo Advisor + Filter Lab backend boundaries, debug/internal gates, consent/quota requirements, request/response guardrails, and R5 exit criteria with no runtime changes.
7. `PT2-SF-R5 - Debug-only iOS Inspiration Backend Integration Scaffold`: implemented debug-only selected-photo Filter Lab scaffold; keeps production/default mock/local, Camera local-only, and no iOS provider key/direct URL.
8. `PT2-SF-R5-R1 - Xcode Swift Concurrency Hotfix`: implemented a narrow compile fix for CloudAI DTO/client isolation after the R5 scaffold; no backend runtime, provider run, credentials, Camera cloud entry, or production rollout.
9. `PT2-SF-R6 - Backend SiliconFlow App Endpoint Routing Fix`: implemented backend-only SiliconFlow provider/routing for internal/debug Photo Advisor and Filter Lab app endpoints, with mocked tests only; no Swift/Xcode change, provider credential commit, live network smoke, or production rollout.
10. `PT2-SF-R7 - Filter Lab Two-image Apply Preview UX`: implemented app-side two-image Filter Lab flow; recipe generation uses the style reference image, local preview rendering applies the recipe to the separate original/apply image, and backend payload remains one reference image only.
11. `PT2-SF-R7-VERIFY - MacBook/Xcode Filter Lab Two-image Apply Preview Verification`: user verified on physical device on 2026-06-22; provider-backed DEBUG AI behavior was not run.
12. `PT2-SF-R8 - Debug-only Filter Lab Provider-backed One-reference Smoke Approval Gate`: implemented docs-only/no-runtime approval gate; no provider call, key read, upload, backend runtime change, or iOS runtime change.
...
15. `PT2-SF-R8-R2 - Fresh Approved One-reference Provider Smoke Retry`: next recommended phase; requires fresh explicit approval before any second provider-backed smoke.
16. `PT3 - Image Editor / 改圖師 Provider Contract`: separate future backend contract after Photo Advisor and Filter Lab are stable; do not mix image editing into PT2-SF.
...
Depth Anything artifact gate note: `docs/phase-21-c-r1-depth-anything-model-artifact-source-and-xcode-benchmark-harness-gate.md` records the source/license/checksum/local-ignore and Xcode physical-device benchmark harness approval gate. It adds no model artifact, Core ML package, download, inference, benchmark, Camera runtime integration, upload path, or production rollout.

Depth Anything harness draft note: `docs/phase-21-c-r2-depth-anything-local-ignored-artifact-and-xcode-harness-draft.md` records the local-only ignored artifact folder policy and Xcode benchmark harness draft. It adds no model artifact, Core ML package, download, inference, benchmark, Camera runtime integration, upload path, or production rollout.
...
- PT2-SF SiliconFlow credential smoke wiring exists for internal/debug Photo Advisor and Filter Lab labels using server-side-only credentials. Text-only smokes accepted in PT2-SF-R1 with `deepseek-ai/DeepSeek-V4-Flash`; PT2-SF-R2 image QA found that model's image request blocked with sanitized `provider_vision_request_rejected`; PT2-SF-R2B selected `Qwen/Qwen3-VL-32B-Instruct`; PT2-SF-R2C accepted one sanitized synthetic Photo Advisor image QA call with that candidate; PT2-SF-R3 accepted one sanitized synthetic Filter Lab structured recipe QA call with that candidate; PT2-SF-R4 completed the debug-only iOS Inspiration backend integration plan without runtime changes; PT2-SF-R5 added the debug-only selected-photo Filter Lab scaffold without provider credentials or provider runs; PT2-SF-R6 fixed backend app endpoint provider routing for `siliconflowInternal` with mocked tests only; PT2-SF-R7 updated Filter Lab to use a style reference image plus a separate original/apply image while keeping backend payload to one reference image.
- The active post-PT2-SF implementation mainline is `PT2-SF-R8-R2`, a fresh-approval retry step after R8-R1 corrected the ignored local backend debug env gate without running a second smoke. Camera AI and small Camera polish are not the current mainline.
...
- Phase 21-Z2B-SF adds backend-only SiliconFlow no-runtime provider contracts. It defines provider/model enums, a fail-closed config schema, JavaScript / Node.js request-shape builder for the operator-provided OpenAI-compatible `/chat/completions` path, synthetic response parser/validator tests, sanitized provider error buckets, and a fail-closed readiness CLI. It adds no provider runtime execution, provider SDK, API call, API key, provider account, image upload, model call, benchmark, inference endpoint call, RunPod provisioning, local Qwen3 install/download/load/call, iOS runtime change, upload payload change, raw artifact, secret, or production rollout. The next recommended phase is Phase 21-Z2C-SF Approval Request Draft for SiliconFlow 12-fixture API Benchmark.
- Phase 21-Z2C-SF drafts the future SiliconFlow 12-fixture API benchmark approval request. It defines exact scope, copyable approval wording, operator checklist, local ignored prerequisites, sanitized aggregate output format, failure gates, and success criteria. It adds no provider API call, API key creation/read/print/commit, provider account, provider runtime execution, image upload, model call, benchmark, inference endpoint call, RunPod provisioning, local Qwen3 install/download/load/call, iOS runtime change, upload payload change, raw artifact, secret, or production rollout. The next recommended phase is Phase 21-Z2C-SF-RUN: Approved SiliconFlow 12-fixture API Benchmark, requiring separate explicit approval.
- Phase 21-Z2C-SF-RUN-PRE adds the missing SiliconFlow benchmark dry-run gate. It defines a deterministic no-runtime benchmark plan, a sanitized dry-run CLI, tests for the exact `smoke_004` through `smoke_015` fixture-token scope, and package scripts for no-network verification. It adds no provider API call, API key creation/read/print/commit, provider runtime execution, image upload, fixture image read, model call, real benchmark, iOS runtime change, upload payload change, raw artifact, secret, or production rollout. The next safe phase remains Phase 21-Z2C-SF-RUN retry only after the dry-run gate passes and the recorded approval/prerequisites remain satisfied.
- Phase 21-Z2C-SF-RUN completed the approved SiliconFlow 12-fixture API benchmark. It used SiliconFlow, `Qwen/Qwen3-VL-30B-A3B-Instruct`, fixtures `smoke_004` through `smoke_015`, planned call count `12`, actual call count `12`, retry `0`, image detail `low`, stream `false`, max output tokens `256`, JavaScript / Node.js backend only, and sanitized aggregate output only. Result: accepted `0`, rejected `12`, validation bucket `provider_schema_invalid x12`, fallback bucket `provider_validation_rejected x12`, latency bucket `5s_to_15s x12`, token usage bucket `lte_20k`, cost bucket `usage_available_cost_not_computed`. It printed/persisted no raw provider response, raw prompt, raw request payload, raw image/base64/path, API key, provider credential, or raw report; changed no iOS runtime/upload payload; added no live cloud AI/app-facing/production endpoint; and kept `productionReady:false`.
- Phase 21-Z2D-SF reviews the SiliconFlow benchmark result without rerunning API calls. It classifies the result as `provider_output_schema_alignment_failed`, keeps SiliconFlow and `Qwen/Qwen3-VL-30B-A3B-Instruct` as the primary provider/model candidates, keeps RunPod as fallback/comparison, and blocks immediate rerun, alternative model benchmark, iOS integration, upload payload change, and production rollout. The next safe phase is Phase 21-Z2D-SF-R1 prompt/schema alignment without API calls.
- Phase 21-Z2E-SF-AUTO performs the explicitly approved bounded SiliconFlow prompt/schema alignment and retry. It adds a backend prompt contract using existing schema enums, improves sanitized parser diagnostics, adds synthetic prompt/schema tests, then runs one approved canary plus one approved full controlled run. Result: total provider calls `13` of cap `28`, canary calls `1`, full run count `1`, retry `0`, accepted `13`, rejected `0`, latency buckets `5s_to_15s x9` and `gt_15s x4`, token usage bucket `lte_20k`, cost bucket `usage_available_cost_not_computed`. It commits no raw provider response, raw prompt, request payload, raw image/base64/path, API key, provider credential, or raw report; changes no iOS runtime/upload payload; adds no live cloud AI/app-facing/production endpoint; and keeps `productionReady:false`.
- Phase 21-Z2F-SF-LATENCY runs operator-approved single-image latency probes on `smoke_004` only. It makes `11` total provider calls with retry `0`, finds compact prompt + `max_tokens:192` as the safest accepted optimization, records `enable_thinking:false` as invalid for this VLM request shape, leaves JSON mode disabled, and changes the backend no-runtime contract default to compact prompt + `maxOutputTokens:192`. Best accepted probe was `5466ms`, while post-patch validation was `10473ms`; this does not support `0.15s` end-to-end SiliconFlow serverless latency. No raw provider output/prompt/request/image/key/report, iOS runtime change, upload payload change, live cloud AI/app-facing/production endpoint, or production rollout is added.
- Phase 21-Z2G-SF runs backend-only network/parameter latency probes after operator suggestions. It makes `5` successful escalated provider calls with `smoke_004`, retry `0`, and sanitized output only. Sequential same-process `fetch` compact `max_tokens:192` remains accepted at `8637ms` and `8529ms`; `stream:true` reaches first chunk at `3979ms` but full validated JSON at `9898ms`; ultra-short `max_tokens:80` and `50` fail schema. No raw provider output/prompt/request/image/key/report, iOS runtime change, upload payload change, live cloud AI/app-facing/production endpoint, or production rollout is added.
