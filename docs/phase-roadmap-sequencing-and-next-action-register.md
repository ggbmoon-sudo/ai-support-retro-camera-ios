...
Reason: Phase 21-W-GOAL-R2-R2 diagnosed the post-readiness `local_model_unavailable x12` result without model calls or benchmark reruns. The primary root cause bucket is `fixture_lookup_mismatch`: the external contract echo allowlist accepts `smoke_004` through `smoke_015`, but the real route still has a separate fixture routeability/image lookup path. The backend benchmark wrapper now preserves sanitized HTTP error buckets instead of collapsing all HTTP non-OK responses to `local_model_unavailable`. A new no-model route-contract dry-run was added in both the external workspace and backend, but the live running external process still returned `route_not_found` for the new endpoint, so the next safe step is a dry-run-only follow-up.
