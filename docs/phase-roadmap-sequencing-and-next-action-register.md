...
Reason: Phase OD-R7E adds backend/local-only expected range comparison for the sanitized OD-R7D-R1 `calibration_001` feature vector. The next local-AI quality step may aggregate multiple approved local calibration outputs after manual review, while still keeping image reads approval-gated, sanitized, and ignored. Do not proceed with Xiaoyi relay credential smoke, Cloud AI relay work, Camera live cloud AI, iOS provider/network code, model files, Swift runtime, committed real photos/local configs/generated reports, or production rollout in this branch unless explicitly requested later.
...
Implementation status note: `docs/od-r7d-local-cv-calibration-smoke.md` records the backend/local-only ignored local fixture calibration smoke. The default CLI safely blocks when ignored config/fixture prerequisites are absent; an explicitly approved local smoke may read only `calibration_001`, emit sanitized feature buckets, and write no reports while keeping provider/cloud/Xiaoyi relay calls, network, uploads, model files, committed real photos/local configs/generated reports, iOS runtime integration, app transfer, and `productionReady` set to true blocked.

Implementation status note: `docs/od-r7e-local-cv-expected-range-comparison.md` records the backend/local-only expected range comparison for sanitized OD-R7D calibration output. The default CLI compares inline sanitized feature vectors only, reads no images, runs no CV inference, emits soft warning buckets rather than scores, writes no reports, and keeps provider/cloud/Xiaoyi relay calls, network, uploads, model files, committed real photos/local configs/generated reports, iOS runtime integration, app transfer, and `productionReady` set to true blocked.
...
Depth Anything preflight plan note: `docs/phase-21-c-depth-anything-v2-small-coreml-sandbox-summary.md` records docs-only sandbox planning. It adds no backend gate/script/tests, Swift runtime file, Depth Anything model artifact, Core ML package, inference execution, benchmark run, Camera runtime integration, upload path, provider call, or production rollout.
