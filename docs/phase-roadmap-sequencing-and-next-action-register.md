...
**MacBook/Xcode physical-device verification for Phase 21-A3-R4**

Reason: Phase 21-A3-R4 fixes the current Camera runtime bug report: non-original filters should be visible in the live viewfinder before shutter, the live filtered view should match the post-capture filtered result, and front-camera preview mirroring should stay separate from the new final-save mirror toggle. The next practical step is MacBook/Xcode physical-device verification that the Metal/Core Image live preview does not drop frames badly, Soft Warm 400 and other filters visibly apply in real time, the saved front-camera image follows the mirror toggle, and Phase 21-A3-R3-R1 focal crop behavior still works. Phase 21-C-R3-RUN remains available only if local ignored artifact prerequisites and the exact approval phrase are present.

If Phase 21-A3-R4 has real-device issues, use a focused `Phase 21-A3-R4-R1 - Camera Filter and Selfie Mirror QA Follow-up` before continuing larger AI roadmap work. If it passes, return to the roadmap decision point. This path should remain free/on-device/local-only unless separately expanded. Do not add Depth Anything runtime, Core ML inference, Florence-2, cloud calls, frame uploads, upload payload changes, Camera live cloud AI, provider keys, sensitive inference, raw frame/depth/image persistence beyond in-memory rendering, or production rollout. `productionReady:false` remains locked.
