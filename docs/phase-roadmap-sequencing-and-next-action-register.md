...
**MacBook/Xcode physical-device verification for Phase 21-A3-R4-R2**

Reason: Phase 21-A3-R4-R2 completes the current selfie parity follow-up: front-camera capture now defaults to mirror-save on so the captured/selected selfie matches the mirror-like viewfinder, and the selected-photo screen adds a local flip-photo button so the user can correct selfie orientation after capture. It builds on Phase 21-A3-R4-R1 orientation stabilization for the Metal/Core Image filtered preview. The next practical step is MacBook/Xcode physical-device verification that switching to the front camera keeps the live preview upright and mirror-like, captured selfies match the viewfinder by default, the post-capture flip button reverses the selected camera photo, filters still render correctly, and Phase 21-A3-R3-R1 focal crop behavior still works. Phase 21-C-R3-RUN remains available only if local ignored artifact prerequisites and the exact approval phrase are present.

If Phase 21-A3-R4-R2 still has real-device orientation/parity/performance issues, keep using focused Camera QA follow-ups before continuing larger AI roadmap work. If it passes, return to the roadmap decision point. This path should remain free/on-device/local-only unless separately expanded. Do not add Depth Anything runtime, Core ML inference, Florence-2, cloud calls, frame uploads, upload payload changes, Camera live cloud AI, provider keys, sensitive inference, raw frame/depth/image persistence beyond in-memory rendering, or production rollout. `productionReady:false` remains locked.
