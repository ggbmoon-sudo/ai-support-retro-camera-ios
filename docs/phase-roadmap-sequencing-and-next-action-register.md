...
Reason: PT2-SF-R4 completed the planning-only selected-photo Inspiration backend integration plan in `docs/pt2-sf-r4-debug-ios-inspiration-backend-integration-plan.md`. PT2-SF-R2C and PT2-SF-R3 already provide accepted one-image backend QA evidence for Photo Advisor and Filter Lab with `Qwen/Qwen3-VL-32B-Instruct` (`qwen3_vl_32b_instruct`). The next mainline should be a small debug-only integration scaffold for selected-photo Inspiration Photo Advisor and/or Filter Lab, using the backend boundary only and keeping production/default mock/local. It must not put provider keys or SiliconFlow URLs in iOS. No Camera AI, Camera polish, Depth Anything, live upload, WSS, image editor provider, direct iOS provider call, provider key in iOS, provider run, or production rollout should be included unless the user explicitly approves that separate scope.
...
7. `PT2-SF-R5 - Debug-only iOS Inspiration Backend Integration Scaffold`: next recommended phase; implement only a small debug/internal selected-photo scaffold; keep production/default mock/local and keep Camera local-only.
8. `PT3 - Image Editor / 改圖師 Provider Contract`: separate future backend contract after Photo Advisor and Filter Lab are stable; do not mix image editing into PT2-SF.
