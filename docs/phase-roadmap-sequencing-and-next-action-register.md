...
Reason: Phase 21-W-FINAL-R1 attempted to reconnect the existing local/private Transformers+FastAPI reference runtime without model calls or benchmarks. External static contracts still passed, the expected local/private port was not listening, and a safe restart was attempted. A listener briefly appeared, but backend healthz remained `connection_refused` and the listener exited afterward. The latest blocker bucket is `startup_failed_after_model_load`.
