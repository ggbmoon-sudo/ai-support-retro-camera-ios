...
**MacBook/Xcode physical-device verification for Phase 21-A3-R3**

Reason: Phase 21-A3-R3 replaces the earlier PiP-style focal preview with the requested framing-and-cropping approach. The next practical step is MacBook/Xcode physical-device verification that the Camera preview is no longer zoomed, the focal box is a transparent movable guide only, the image inside the box is not magnified during preview, the slider changes box size/focal crop, and the captured result is only the box area. Phase 21-C-R3-RUN remains available only if local ignored artifact prerequisites and the exact approval phrase are present.

If Phase 21-A3-R3 focal crop has real-device issues, use a focused `Phase 21-A3-R4 - Focal Crop Real-device QA Follow-up` before continuing larger AI roadmap work. If it passes, return to the roadmap decision point. This path should remain free/on-device/local-only unless separately expanded. Do not add Depth Anything runtime, Core ML inference, Florence-2, cloud calls, frame uploads, upload payload changes, Camera live cloud AI, provider keys, sensitive inference, raw frame/depth/image persistence beyond normal captured photo flow, or production rollout. `productionReady:false` remains locked.
