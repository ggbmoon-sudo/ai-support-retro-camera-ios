...
Reason: Phase 21-W-GOAL-R2-R2A followed up the live no-model route-contract dry-run without model calls or benchmark reruns. External static route-contract and fixture-token checkers still passed, and the backend live dry-run initially still returned `route_not_found`, so the route-not-found cause bucket is `stale_server_process`. The local/private listener was restarted through the approved helper, but the required post-reload backend healthz preflight blocked with `connection_refused`, so the backend live route dry-run was not rerun.
