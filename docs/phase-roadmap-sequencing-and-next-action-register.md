...
Reason: PT2-SF-R5 added the debug-only selected-photo Filter Lab backend scaffold while keeping production/default mock/local. The next mainline should be operator MacBook/Xcode verification of the DEBUG UI path against the local backend boundary, starting with disabled/fallback behavior and no provider credentials. A provider-backed debug smoke still requires separate explicit approval and should be one selected image only. No Camera AI, Camera polish, Depth Anything, live upload, WSS, image editor provider, direct iOS provider call, provider key in iOS, unapproved provider run, or production rollout should be included unless the user explicitly approves that separate scope.
...
8. `PT2-SF-R5-VERIFY - MacBook/Xcode Debug Inspiration Backend Smoke`: next recommended phase; verify DEBUG selected-photo Photo Advisor / Filter Lab backend boundary and fallback behavior on MacBook/Xcode; no provider-backed smoke without separate explicit approval.
9. `PT3 - Image Editor / 改圖師 Provider Contract`: separate future backend contract after Photo Advisor and Filter Lab are stable; do not mix image editing into PT2-SF.
