...
- Next planned phase after Phase 19-D: `Phase 19-E`, if explicitly requested. It should remain backend-only / local-synthetic unless the prompt explicitly approves a real local model benchmark.
- Phase 19-E must not add iOS app integration, model server URLs, provider/model credentials, real-provider QA, real VLM calls, model downloads, model cache changes, backend provider request payload changes, iOS upload payload changes, capture-context upload, Camera cloud AI, provider/model keys in iOS, training/fine-tuning, user-photo training, or production readiness unless separately approved.
