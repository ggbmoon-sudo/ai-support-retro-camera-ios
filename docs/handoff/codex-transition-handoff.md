...
- Next planned phase after Phase 19-A: `Phase 19-B: Open-weight VLM Local Sandbox / Benchmark Plan`.
- Phase 19-B should remain planning / sandbox-only unless explicitly expanded. It should define ignored model-cache / sandbox artifact paths, hardware assumptions, candidate benchmark commands, synthetic-first evaluation, optional approved ignored local samples, sanitized aggregate notes, no iOS app integration, no backend provider request payload change, no iOS upload payload change, no capture-context upload, no Camera cloud AI, no real-provider QA requirement, and `productionReady=false`.
