...
- Recommended next work is the current next phase in `docs/phase-roadmap-sequencing-and-next-action-register.md`: Phase 21-L, Local On-device CV Camera Aids Plan. Keep it planning/gate-only unless the prompt explicitly approves runtime local CV implementation, upload runtime, WSS runtime, Auto-Trigger runtime, iOS runtime, endpoint, model call, serving benchmark, or production work.
