...
- Next planned phase after Phase 19-B: `Phase 19-C: Open-weight VLM Local Benchmark Harness Plan`.
- Phase 19-C should remain harness-planning / sandbox-only unless explicitly expanded. It may define ignored model-cache / sandbox artifact paths, synthetic fixture files, candidate schema/validator implementation only if explicitly requested, docs-only or no-network benchmark commands, sanitized aggregate report shape, and no iOS app integration. It must not run real models, change backend provider request payloads, change iOS upload payloads, upload capture context, add Camera cloud AI, add provider/model keys to iOS, start training/fine-tuning, or mark production ready unless separately approved.
