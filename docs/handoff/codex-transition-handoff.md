...
- Recommended next work is the current next phase in `docs/phase-roadmap-sequencing-and-next-action-register.md`: Phase 21-K, Stateful WSS Live Advisor Protocol Preflight. Keep it docs/gate/schema-only unless the prompt explicitly approves WSS runtime, Auto-Trigger runtime, upload runtime, iOS runtime, endpoint, model call, serving benchmark, or production work.
