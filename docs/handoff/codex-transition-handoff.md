...
- Recommended next work is the current next phase in `docs/phase-roadmap-sequencing-and-next-action-register.md`: Phase 21-M, Quantization + Serving Benchmark Plan. Keep it planning/gate-only unless the prompt explicitly approves benchmark execution, runtime local CV implementation, upload runtime, WSS runtime, Auto-Trigger runtime, iOS runtime, endpoint, model call, Qwen inference, vLLM/SGLang/Ollama call, serving-stack switch, or production work.
- Do not enable `local_model`, run vLLM/SGLang/Ollama benchmarks, switch model stacks, run more real smoke, add iOS integration, add endpoints, add auth/billing/quota runtime, run Qwen inference, run fixture inference, or change `productionReady:false` without explicit future approval.
