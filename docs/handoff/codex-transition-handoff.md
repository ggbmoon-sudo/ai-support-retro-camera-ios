...
- Recommended next work is Phase 21-G only if explicitly requested after the deployment config/env preflight. A safe candidate is backend-internal fallback/failure taxonomy mapping for contract, route, healthz, echo, validation, privacy, deployment-boundary, and config/env blockers.
- Do not run vLLM/SGLang/Ollama benchmarks, switch model stacks, run more real smoke, add iOS integration, add endpoints, add auth/billing/quota runtime, run Qwen inference, run fixture inference, or change `productionReady:false` without explicit future approval.
