...
- Next planned phase after Phase 18-C4: `Phase 19-A: Open-weight VLM Backend Architecture ADR`.
- Phase 19-A should be architecture/research only unless explicitly expanded. It should compare Qwen2.5-VL / Qwen2-VL / MiniCPM-V candidates, Ollama prototype vs vLLM/SGLang production-style serving, structured JSON output, backend validation/fallback, explicit post-capture consent only, no raw image/prompt/provider-response logging, no user-photo training without explicit consent, and iOS remaining free of provider keys / direct model calls.
