...
Latest Phase OD-R4 Cloud AI Teacher Contract on 2026-06-21:

- OD-R4 adds a backend-only Cloud AI Teacher request/response contract for future offline structured labeling.
- Added `backend/src/qa/aestheticCloudTeacherContract.mjs`, `backend/scripts/check-aesthetic-cloud-teacher-contract.mjs`, backend tests, `npm run qa:aesthetic-teacher:contract`, and `docs/od-r4-cloud-ai-teacher-contract.md`.
- The contract accepts structured request fields only: opaque job/image IDs, registry version, allowed tag subset, asset reference type/bucket, source type, `teacherMode:"contract_stub_only"`, and `humanReviewRequired:true`.
...
Latest Phase OD-R5C Cloud Teacher Provider Smoke Harness on 2026-06-21:

- OD-R5C adds a backend-only opt-in Cloud Teacher provider smoke harness while keeping the default path no-network and fail-closed.
- Added `backend/src/qa/aestheticCloudTeacherProviderSmokeHarness.mjs`, `backend/src/qa/aestheticCloudTeacherProviderAdapter.mjs`, `backend/scripts/run-aesthetic-cloud-teacher-provider-smoke-harness.mjs`, backend tests, `npm run qa:aesthetic-teacher:provider-smoke-harness`, and `docs/od-r5c-cloud-teacher-provider-smoke-harness.md`.
- The harness uses the OD-R5B readiness gate and redacted envelope, validates represented output through the OD-R4 teacher contract, and requires OD-R6B human review queue routing before any future tuning or app-transfer discussion.
...
Latest Phase OD-P1A App Transfer Preflight Gate on 2026-06-21:

- OD-P1A adds a backend-only App Transfer Preflight Gate using a safe inline synthetic OD-R8B export candidate and app contract only.
- Added `backend/src/qa/aestheticAppTransferPreflightGate.mjs`, `backend/scripts/check-aesthetic-app-transfer-preflight-gate.mjs`, backend tests, `npm run qa:aesthetic-parameters:app-transfer-preflight`, and `docs/od-p1a-app-transfer-preflight-gate.md`.
- The gate validates the candidate through OD-R8B first, then checks app contract compatibility, language-pack-only UI copy, CreativeIntentGuard, offline fallback, and kill switch requirements.
- Output reports `runMode:"app_transfer_preflight"`, `preflightPassed:true`, `eligibleForAppTransferCandidate:true`, `eligibleForAppRuntime:false`, `appRuntimeTransferBlocked:true`, `appRuntimeWritePerformed:false`, `xcodeProjectModified:false`, `appBundleArtifactWritten:false`, `cvInferencePerformed:false`, `trainingEnabled:false`, `fineTuningEnabled:false`, `runtimeIntegrationEnabled:false`, and `productionReady:false`.
...
Latest Phase OD-R8F Parameter-pack Candidate Builder on 2026-06-23:

- OD-R8F builds a backend/local-only review scaffold from OD-R8E accepted candidates only.
- Added `backend/src/qa/aestheticParameterPackCandidateBuilder.mjs`, `backend/scripts/run-aesthetic-parameter-pack-candidate-builder.mjs`, backend tests, `npm run qa:aesthetic-parameters:pack-candidate-builder`, and `docs/od-r8f-parameter-pack-candidate-builder.md`.
- Included `horizonAngle.softTiltThreshold` and `visualWeightMoment.balanceWarningPolicy`; excluded `headroomRatio.highSoftWarningThreshold` and `sharpnessRatio.lowConfidenceThreshold` as `needs_more_data`.
- Output reports `runMode:"parameter_pack_candidate_builder_no_export"`, `candidateOnly:true`, `acceptedForPackCandidateReview:true`, `eligibleForParameterPackExport:false`, `eligibleForAppRuntime:false`, `parameterPackExported:false`, `appRuntimeWritePerformed:false`, and `productionReady:false`.
- Parameter-pack export and app runtime remain blocked; no image read, CV inference, provider/cloud/Xiaoyi relay call, network/upload/download/crawler, report write, real fixture/config/report commit, Swift/Xcode change, model file, training/fine-tuning, Camera behavior change, or production rollout is added.
...
- Added `backend/src/qa/aestheticLocalCvFeatureExtractorPrototype.mjs`, `backend/scripts/run-aesthetic-local-cv-feature-extractor-prototype.mjs`, backend tests, `npm run qa:aesthetic-local-cv:feature-extractor`, and `docs/od-r7b-local-cv-feature-extractor-prototype.md`.
- The extractor maps synthetic observations for `headroomRatio`, `horizonAngle`, `highlightClipRatio`, `edgeMargin`, `backgroundObjectDensity`, `sharpnessRatio`, `subjectAnchor`, and `visualWeightMoment` into OD-R7A-compatible feature vectors.
