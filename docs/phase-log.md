...
Current phase: Phase 21-A4 - On-device Live Framing Hint UX Pass
Status: implemented, pending Xcode physical-device verification
Latest implementation: Phase 21-A4 improves the local-only live framing hint UX on top of the existing Apple Vision / AVFoundation geometry and depth signal path. The expanded local guidance panel now uses a `Next hint` action instead of debug-style state cycling, users can manually rotate visible local hints, and compact/expanded hints use category-specific icons for lighting, composition, background, portrait, and filters. It adds no Depth Anything runtime, Core ML model inference, Florence runtime, cloud AI, provider call, preview-frame upload, upload payload change, raw frame persistence beyond in-memory rendering, or production rollout. `productionReady:false` remains locked.
