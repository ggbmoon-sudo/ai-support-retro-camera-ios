...
Current phase: Phase 21-A3-R3 - Focal Framing Box Crop
Status: completed
Latest implementation: Phase 21-A3-R3 replaces the earlier dual-video / PiP-style focal preview with a simple transparent framing-box crop. The Camera preview defaults back to `resizeAspect` so the viewfinder is not zoom-cropped; the box is only a movable UI guide with no live magnification inside it; on shutter, the app captures the full-resolution photo and crops the saved/selected result to the box area. It adds no Depth Anything runtime, Core ML inference, cloud AI, provider call, preview-frame upload, upload payload change, raw frame persistence beyond normal captured photo flow, or production rollout. `productionReady:false` remains locked.
