...
The user explicitly approved: "?????Phase 21-N-R1 ?鞊啣????backend local/private model smoke retry". Phase 21-N-R1 stopped before healthz/model execution because the ignored local config fixture token was not `smoke_001`. No substitute fixture token was used.
...
The user explicitly approved: "?????Phase 21-N ?鞊啣????backend local/private model smoke". The phase stopped before any healthz/model call because required preferred fixture token `smoke_001` was not present/approved in the ignored local fixture registry. No substitute fixture token was used.
...
- Updated imported-photo limited-context copy so it says capture details / ????????? / ?????食????/ ????????? instead of raw ???爽text??
- Softened provider-unavailable fallback copy from explicit Cloud AI wording to calmer remote / ????????? local fallback language.
...
- English copy was mostly short and safe; visible ??謆k demo??wording was too implementation-facing for beta UI.
- Traditional Chinese fallback copy was generally usable but mixed in raw ???爽text??and ??姿?????AI??technical language.
...
- Polished user-facing fallback/error copy so it no longer says ??謆k advisor?? ???築alid result?? or ????fe fallback filter??in visible UI.
...
- Added required closeout sections for future Codex final responses, including `?????/ What changed` and `Xcode ???????????? / What to check in Xcode`.
...
Make Post-capture Photo Advisor / AI ?????/ ?????西??? follow the same persisted Language / Tone preference introduced in Phase 16V, while keeping the feature mock/local-only.
...
- Recorded the deterministic copy system goal and the three core principles: ?雓?????????????????????; ?雓???????????????? ?????????????????????????
...
- Preserved the provided report title: `Hong Kong / ??????Language Mode UX + Safety for iOS Retro Camera App ??????????????`.
...
- Preserved the required sections:
  ...
  - Final Recommendation
  - Sources / Links
- Recorded the research conclusion that ????謇游??/ ??????mode is a Hong Kong localization brand personality, not a generic AI tone pack, translation mode, or profanity feature.
...
- Preserved the provided report title: `Paid AI Image Editing / ?????for iOS Retro Camera App ??????????????`.
- Saved the full ChatGPT-provided Markdown research report to the repo.
- Removed `utm_source=chatgpt.com` query strings without changing source meaning.
- Preserved the required sections:
  - Executive Summary
...
- Recorded that user-facing UI may call it ?????/ ???????????/ ??????????????/ Photo Fix / Style Edit and can reduce everyday `AI` wording.
...
- Preserved the provided report title: `Encrypted App-to-App High Quality / Lossless Photo Transfer for iOS Retro Camera App ??????????????`.
- Saved the full ChatGPT-provided Markdown research report to the repo.
- Removed `utm_source=chatgpt.com` query strings without changing source meaning.
- Preserved the required sections:
  - Executive Summary
...
- Preserved the provided report title: `Local On-device Camera Coach + LiDAR Scene Understanding for iOS Retro Camera App ??????????????`.
- Saved the full ChatGPT-provided Markdown research report to the repo.
- Removed `utm_source=chatgpt.com` query strings without changing source meaning.
- Preserved the required sections:
  - Executive Summary
...
- Keep categories inclusive: `???` / `????` / `???Ⅹ雓 are browsing labels only, not user classification.
...
Phase 21-C adds a disabled-by-default iOS Depth Anything V2 Small sandbox scaffold for future Core ML depth fallback benchmarking. It is not connected to Camera runtime and does not include a model file, Core ML package, model download, inference execution, or benchmark run.

### Completed Work

- Added `DepthAnythingV2SmallSandbox.swift`.
- Added debug-only readiness/configuration contracts for Depth Anything V2 Small sandbox gating.
- Added sanitized benchmark sample buckets for model load, first/warmed inference, memory, FPS impact, thermal state, battery drain, depth stability, invalid output rate, and app-size impact.
- Kept hardware AVFoundation depth first.
- Kept sandbox disabled by default and blocked on missing model artifact, serious/critical thermal state, Low Power Mode, raw logging, persistence, or `productionReady:true`.
- Added `docs/phase-21-c-depth-anything-v2-small-coreml-sandbox-summary.md`.
- Updated roadmap, README, backend/iOS README, handoff, and manual smoke notes.

### Changed Files

- `ios-app/AIPhotoApp/Features/Camera/DepthAnythingV2SmallSandbox.swift`
...
Phase 21-C-R3 adds an approval-request draft for a future Depth Anything V2 Small physical-device benchmark. It does not approve execution by itself and adds no model artifact, Core ML package, download, inference, benchmark, Camera runtime integration, upload path, raw artifact, or production rollout.

### Completed Work

- Added `docs/phase-21-c-r3-depth-anything-operator-artifact-verification-and-physical-device-benchmark-approval-request.md`.
- Recorded exact future C-R3-RUN approval wording.
- Defined source/license/model-card/checksum/local-ignore prerequisites.
- Defined physical-device-only Xcode benchmark scope and simulator block.
- Defined hardware AVFoundation depth first policy.
- Defined sanitized aggregate output format, failure gates, and success criteria.
...
- Backend Depth Anything gates should still pass.
- Artifact scans should show no model files, local configs, raw reports, real photos, datasets, or provider responses staged.
- Xcode runtime behavior should be unchanged; physical-device benchmark is not run in this phase.
...
Phase 21-A3 tunes the already-running on-device Apple Vision live guidance path after physical-device feedback confirmed brightness guidance and Vision geometry guidance work, but geometry hints felt not fully smooth.

### Completed Work

- Reduced frame analysis interval from `0.6s` to `0.5s`.
- Increased hint hold duration from `2s` to `3s`.
- Increased repeat cooldown from `4s` to `6s`.
- Increased replacement confirmation from `2` to `3` candidate batches.
- Added `1.2s` minimum visible duration before replacing stable hints.
- Raised body-pose confidence from `0.2` to `0.35` and required at least `4` points before body bounds are accepted.
- Relaxed center, edge, headroom, footroom, subject-size, and rule-of-thirds thresholds to reduce jitter.
- Removed automatic positive rule-of-thirds / vertical-balance ready hints so the UI does not flip when no action is needed.
- Added `docs/phase-21-a3-live-vision-geometry-smoothness-and-hint-stability-tuning.md`.
...
- `docs/on-device-live-framing-ai-roadmap.md`
- `docs/phase-roadmap-sequencing-and-next-action-register.md`
- `docs/phase-21-a3-live-vision-geometry-smoothness-and-hint-stability-tuning.md`
- `docs/phase-log.md`
- `docs/handoff/codex-transition-handoff.md`
- `tests/manual-smoke-tests.md`
...
- Model call / provider call: no
- Camera cloud AI entry: no
- Preview-frame upload: no
- Upload payload changed: no
- Raw frame/depth persistence: no
- Sensitive inference added: no
- `productionReady:false` remains locked.

### Xcode Verification Needed

Build and run on iPhone. Confirm lighting hints still respond, geometry hints feel less jumpy near edges/headroom/subject-size changes, suggestions hold before changing, and no upload/provider/model/depth-runtime behavior appears.

### Ready for Next Phase

Ready for `Phase 21-A3-R1 - Physical-device Live Guidance Tuning Feedback Pass` after the user tests this build. Not ready for production rollout.
...
Phase 21-A3-R1 fixes real-device Camera bugs reported from iPhone testing: front-camera toggle did not switch the camera, lens mm controls did not switch the camera/lens, the viewfinder appeared zoomed versus captured output, filters were not visible in the live viewfinder before capture, and flash did not fire.

### Completed Work

- Added real AVFoundation input switching for front/back camera toggle.
- Added device-detected lens options so unavailable lenses, such as telephoto on devices without telephoto hardware, are not shown.
- Wired lens selection into session input reconfiguration instead of only updating UI state.
- Set preview layer framing to aspect-fit and mirrored front-camera preview.
- Updated the framed preview path to a 3:4 aspect ratio.
- Added lightweight live filter preview styling for the viewfinder while preserving the existing Core Image render path after capture.
- Passed flash state into `AVCapturePhotoSettings` and kept screen flash for front camera.
- Added capture-context handling for the front selfie lens bucket.
- Updated lens accessibility strings to remove mock labels.
- Added `docs/phase-21-a3-r1-real-device-camera-controls-and-preview-fixes.md`.
...
- `README.md`
- `ios-app/README.md`
- `docs/phase-21-a3-r1-real-device-camera-controls-and-preview-fixes.md`
- `docs/phase-log.md`
- `docs/handoff/codex-transition-handoff.md`
- `tests/manual-smoke-tests.md`
...
Build and run on a physical iPhone. Confirm front/back switching, device-specific lens options, actual lens switching, viewfinder/capture framing alignment, live filter preview before capture, back-camera flash, and front-camera screen flash.
...
## Phase 21-A3-R2-R1 - Xcode Warning and Compile Cleanup
...
Phase 21-A3-R2-R1 fixes the Xcode issue list surfaced after the adjustable dual focal PiP zoom phase. It is a focused compile/warning cleanup and does not change product scope.

### Completed Work

- Fixed `CameraDualFocalZoomConfiguration.zoomFactor` by calling the static helper with `Self.sanitizedFocalLength`.
- Marked pure Photo Advisor language/filter reason lookup helpers as `nonisolated`.
- Marked private Guidance copy enum helpers as `nonisolated`.
- Removed the main-actor `.shared` default argument from `LocalRuleBasedGuidanceProvider` and resolves the shared store inside the MainActor init.
- Replaced deprecated `AVCaptureConnection.videoOrientation` / `isVideoOrientationSupported` usage with `videoRotationAngle` / `isVideoRotationAngleSupported`.

### Changed Files

- `ios-app/AIPhotoApp/Features/Camera/CameraDualFocalZoom.swift`
- `ios-app/AIPhotoApp/Features/Camera/CameraPreviewView.swift`
- `ios-app/AIPhotoApp/Features/Camera/GuidanceCopyResolver.swift`
- `ios-app/AIPhotoApp/Features/Camera/LocalRuleBasedGuidanceProvider.swift`
- `ios-app/AIPhotoApp/Features/AIPhotoAdvisor/PostCapture/PhotoAdvisorLanguagePack.swift`
- `ios-app/AIPhotoApp/Features/AIPhotoAdvisor/PostCapture/PhotoAdvisorFilterReasonLibrary.swift`
- `docs/phase-log.md`
...
Build in Xcode and confirm the visible issues for `CameraDualFocalZoom`, `PhotoAdvisorCopyResolver`, `GuidanceCopyResolver`, `LocalRuleBasedGuidanceProvider`, and `CameraPreviewView` are cleared or reduced to unrelated historical warnings.

### Ready for Next Phase

Current practical next step remains MacBook/Xcode physical-device verification of Phase 21-A3-R2 PiP zoom behavior. Not ready for production rollout.

## Phase 21-A3-R2-R2 - Default-on Transparent Dual Focal PiP
...
Phase 21-A3-R2-R2 tunes the dual focal PiP behavior after real-device feedback showed the mode should not require manual enablement and the main viewfinder should remain normal full-frame size behind the PiP inset.

### Completed Work

- Made dual focal PiP default-on when Camera opens.
- Changed the focal chip into a controls entry instead of an enable/disable switch.
- Changed the slider close button to close the controls only, not disable PiP.
- Removed the nested second `CameraPreviewView` from the PiP overlay.
- Added a local in-memory preview frame stream from `AVCaptureVideoDataOutput` for the PiP inset.
- Kept the main camera preview as the only full-size `AVCaptureVideoPreviewLayer`.
- Restored `CameraPreviewView` to full-frame `resizeAspectFill` by default so the camera surface does not become an aspect-fit black canvas when PiP is on.
- Made the PiP inset size respond to focal length: larger near the selected lens focal length and smaller toward `100mm`.
- Kept the area outside the PiP inset transparent so the full environment remains visible.
- Preserved captured-photo PiP rendering with equivalent-FOV crop math.
...
Build and run on iPhone. Confirm the PiP inset appears by default, the main viewfinder stays full-frame and visible behind it, the inset changes size with focal length, the area outside the inset stays transparent, dragging/slider focal control remains continuous, and captured photos include the inset effect.

### Ready for Next Phase

Current practical next step remains MacBook/Xcode physical-device verification of Phase 21-A3-R2 PiP behavior. Not ready for production rollout.
...
Phase 21-A3-R3 replaces the previous dual-video / PiP-style focal preview with the requested framing-and-cropping approach. The Camera preview is no longer zoom-cropped by default, the focal box is a transparent movable UI guide only, and the final captured result is cropped to the box area after full-resolution capture.

### Completed Work

- Restored `CameraPreviewView` default video gravity to `resizeAspect` so the viewfinder is not full-screen zoom-cropped.
- Removed the real-time PiP preview-frame stream and `CameraPreviewFrameSnapshot` path.
- Replaced captured-photo inset rendering with `CameraDualFocalPhotoCropper`.
- Changed the focal overlay into a transparent framing box only.
- Made dragging the box move crop position instead of changing focal length.
- Kept the slider as the focal-length / box-size control.
- Mapped box center and size ratios to captured image pixel crop coordinates.
- Made captured output only the image area inside the box.
- Updated English and Traditional Chinese UI strings from PiP zoom language to focal crop language.
...
- `ios-app/README.md`
- `docs/phase-roadmap-sequencing-and-next-action-register.md`
- `docs/phase-21-a3-r3-focal-framing-box-crop.md`
- `docs/phase-log.md`
- `docs/handoff/codex-transition-handoff.md`
- `tests/manual-smoke-tests.md`
...
- `git diff --check`
- Removed-symbol scan for the old PiP preview-frame path
- Boundary scan for model files, provider/API keys, cloud calls, preview-frame upload, payload changes, and raw artifacts

### Boundary Confirmations

- Real-time dual video rendering: removed
- PiP live magnification: removed
- Provider/model/cloud call: no
- Camera live cloud AI entry: no
- Preview-frame upload: no
- Upload payload changed: no
...
Build and run on iPhone. Confirm the Camera preview is no longer zoomed, the transparent focal box can be moved freely, there is no live magnification inside the box, the slider changes box size, and the captured result is only the box area.

### Ready for Next Phase

Current practical next step remains MacBook/Xcode physical-device verification of Phase 21-A3-R3 focal crop behavior. If crop mapping is still off, use a focused `Phase 21-A3-R4 - Focal Crop Real-device QA Follow-up`. Not ready for production rollout.
...
Phase 21-A3-R3-R1 tunes the focal framing-box crop after real-device feedback that the crop box needed dynamic aspect ratio and state-driven pinch behavior. The box is hidden at the selected lens/base focal length, auto-shows when pinch-out or slider focal length moves above base, clamps so pinch-in cannot go below base, hides again at base, and can be reshaped to `4:5`, `1:1`, or `3:4`.

### Completed Work

- Added a local `CameraFocalCropAspectRatio` enum for `4:5`, `1:1`, and `3:4`.
- Passed the selected aspect ratio into the focal crop configuration, overlay sizing, and captured-photo crop calculation.
- Changed focal crop default state so the framing box is hidden when the selected focal length equals the active lens/base focal length.
- Added pinch gesture handling on the Camera preview so pinch-out increases focal crop and auto-shows the box.
- Added clamp/hide behavior so pinch-in cannot go below the selected lens/base focal length and hides at base.
- Added aspect-ratio chips to the focal crop callout and localized the label in English and Traditional Chinese.
...
Build and run on iPhone. Confirm the Camera preview is no longer zoomed, the focal box is hidden at base focal length, pinch-out automatically shows/resizes the transparent box, pinch-in clamps/hides it at base, `4:5` / `1:1` / `3:4` aspect choices reshape the box correctly, there is no live magnification inside the box, the slider uses the same base-to-`100mm` limits, and the captured result is only the box area.
...
Phase 21-A3-R5 is a focused local Camera runtime polish pass after the user completed MacBook/Xcode physical-device verification of Phase 21-A3-R4-R2. It stabilizes the flash control state, addresses Xcode CameraCaptureService hang-risk/concurrency warnings, and reduces live filtered preview memory pressure without changing the camera preview product behavior, focal crop pipeline, upload payload, or AI/runtime architecture.

### Completed Work

- Added a single effective flash state for capture so unsupported flash is never passed as enabled.
- Kept front-camera flash behavior as local screen flash only.
- Kept back-camera flash behavior hardware-gated by `AVCaptureDevice.hasFlash`.
- Disabled and dimmed flash UI when the active camera has no available flash effect.
- Reset the flash toggle when switching into a camera state where flash is unavailable.
- Moved `AVCaptureSession.startRunning()` / `stopRunning()` onto a dedicated background session queue.
...
Phase 21-A4 improves the local-only live framing hint UX using the existing Apple Vision / AVFoundation signal path. It keeps the Camera guidance free, on-device, short, mood-first, and non-judgmental.

### Completed Work

- Replaced local guidance panel debug-style state cycling with a user-facing `Next hint` action.
- Added manual cycling for currently visible local hint suggestions.
- Added category-specific icons for lighting, composition, background, portrait, and filter hints.
- Updated compact and expanded live guidance surfaces to use the hint category icon.
...
- `ios-app/AIPhotoApp/Resources/Localization/en.lproj/Localizable.strings`
- `ios-app/AIPhotoApp/Resources/Localization/zh-Hant.lproj/Localizable.strings`
- `docs/phase-21-a4-on-device-live-framing-hint-ux-pass-summary.md`
- `docs/phase-roadmap-sequencing-and-next-action-register.md`
- `docs/phase-log.md`
- `README.md`
- `ios-app/README.md`
...
- `git diff --check`
- Focused source scan for provider keys, network calls, upload payload changes, model artifacts, raw frame persistence, and Camera cloud AI entries
- Xcode physical-device build remains required for live hint UX verification.
...
Build and run on iPhone. Confirm compact guidance uses category icons, expanded guidance shows `Next hint` in local mode, tapping `Next hint` cycles visible hints when more than one hint is available, the action is disabled/dimmed when only one hint is available, and filter preview, selfie mirror parity, flash behavior, and focal crop still work.

### Ready for Next Phase

Current practical next step is MacBook/Xcode physical-device verification of Phase 21-A4. If hint UX behavior or Camera runtime stability regresses, continue with a focused Camera QA follow-up. Not ready for production rollout.
...
Phase 21-A5 records the product clarification that richer composition guidance should be owned by the training-AI branch, not expanded inside the on-device Camera runtime.

### Completed Work

- Added `docs/phase-21-a5-composition-guidance-ownership-boundary.md`.
- Clarified that app-side Camera remains responsible for safe local geometry/depth signals, lightweight hint shell UX, and conservative fallback copy only.
- Clarified that richer composition hint generation, learned scene/layout patterns, label schemas, AI-assisted labeling, distillation, and future typed composition outputs belong to the training-AI branch.
- Updated roadmap, README, iOS README, on-device roadmap, handoff, and manual smoke notes.

### Changed Files

- `docs/phase-21-a5-composition-guidance-ownership-boundary.md`
- `docs/phase-roadmap-sequencing-and-next-action-register.md`
- `docs/phase-log.md`
- `docs/on-device-live-framing-ai-roadmap.md`
- `docs/handoff/codex-transition-handoff.md`
...
- `git diff --check`
- Docs/source scan for model files, provider keys, preview-frame upload, upload payload changes, raw frame persistence, dataset crawler, AI-assisted labeling runtime, and production readiness changes
...
- Xcode build/runtime verification was not run in this Windows environment.
- Operator reported physical-device testing OK on 2026-06-22.
...
PT2-SF-R8-R1 corrected the ignored local backend debug env gate after PT2-SF-R8-RUN returned `internal_cloud_disabled`. Only non-secret gate keys were updated in `backend/.env.local`; no tracked backend runtime code was changed.

### Completed Work

- Updated ignored local env gate keys without printing file contents or secret values.
- Preserved provider credential values server-side only.
- Ran a no-network config bucket check.
- Confirmed Filter Lab now resolves to `siliconflowInternal` with `X-Internal-Debug-CloudAI: true`.
- Did not start a backend server.
- Did not call `/v1/ai/filter-lab`.
- Did not run a provider/model call.
- Added `docs/pt2-sf-r8-r1-server-side-debug-env-gate-correction.md`.
...
- Filter Lab gate with debug header: `siliconflowInternal`
...
- `README.md`
- `ios-app/README.md`
- `docs/pt2-sf-r8-r1-server-side-debug-env-gate-correction.md`
- `docs/phase-roadmap-sequencing-and-next-action-register.md`
- `docs/phase-log.md`

Ignored local-only change:

- `backend/.env.local` non-secret gate keys only

### Tests and Checks

- No-network config gate check
- `git diff --check`
- Safety scans for Swift/Xcode/backend runtime drift, provider credential leakage, raw request/provider/image/base64 leakage, direct iOS provider calls, upload payload expansion, and `productionReady:true`
...
None for this local ignored-env/docs closeout. Xcode runtime behavior should remain unchanged. A future provider-backed Filter Lab smoke still requires a fresh explicit approval before any second endpoint request.

### Ready for Next Phase

Recommended next step is `PT2-SF-R8-R2 - Fresh Approved One-reference Provider Smoke Retry`. Require fresh explicit approval because R8-RUN already consumed its one allowed backend endpoint request. Keep the retry DEBUG/backend-mediated only, one style reference image only, no apply/original image upload, no direct iOS provider call, no raw logging, and `productionReady:false`.
...
PT2-SF-R8-R2 ran one fresh-approved DEBUG Filter Lab backend app endpoint smoke after R8-R1 corrected the ignored local server-side debug env gate. The existing `/v1/ai/filter-lab` backend boundary returned `source:cloud` with a generated filter present from one style reference image only.

No Swift, Xcode project, backend runtime code, provider credential, direct iOS provider call, backend payload expansion, Camera AI, image editor provider, StoreKit/quota behavior, or production rollout was added.

### Completed Work

- Confirmed branch `feat/phase-02-auth`, clean worktree, and upstream count `0 0` before the run.
- Confirmed `backend/.env.local` exists and remains ignored without printing secret values.
- Ran a no-network env bucket preflight.
- Confirmed Filter Lab resolves to `siliconflowInternal` with the existing debug header.
- Started a temporary local backend listener on `127.0.0.1:8789`.
- Sent exactly one DEBUG backend app endpoint request to `/v1/ai/filter-lab`.
- Sent one style reference image only.
- Sent no apply/original image.
- Printed only sanitized smoke buckets.
- Stopped the temporary backend listener after the first endpoint response.
- Added `docs/pt2-sf-r8-r2-fresh-approved-one-reference-provider-smoke-retry.md`.
...
None for this docs/report closeout. Xcode runtime behavior should remain unchanged because no Swift or Xcode project file changed.

For the next phase, verify on physical device that the DEBUG Filter Lab UI can reach a phone-accessible local/LAN backend, receive a cloud generated recipe, and apply it locally to the separate apply/original image.
...
PT2-SF-R9-R6 identifies the physical-device launch crash as the known Xcode/iOS debugger issue that reports `-[OS_dispatch_mach_msg _setContext:]: unrecognized selector sent to instance`. This happens before the app's SwiftUI root can render, even with a pure `Text("DEBUG SAFE BOOT")` root.

The app-side temporary safe boot changes were restored. The required operator workaround is:
...
- Select `Run`.
- Open `Options`.
- Uncheck `Enable backtrace recording`.
- Keep `AI_PHOTO_CLOUD_AI_BASE_URL=http://192.168.68.60:8787` only as an Environment Variable if needed for the LAN backend test.
- Clean Build Folder.
