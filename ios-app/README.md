...
Phase 21-A3-R3 replaces the earlier dual-video / PiP-style focal preview with a simple focal framing-box crop. The Camera preview is back to the standard 1x aspect-fit feed so it matches captured-photo framing more closely; the focal box is only a transparent movable UI guide with no live magnification inside it. On shutter, the app captures the full-resolution photo and crops the saved/selected result to the box area. It adds no Depth Anything runtime, Core ML inference, cloud AI, provider key, direct provider/model call, preview-frame upload, upload payload change, raw frame/depth/image persistence beyond normal captured photo flow, or production rollout.

The research index for this direction is `../docs/research/on-device-ai-research-index.md`. Later Depth Anything / Florence work remains blocked until separately approved and benchmarked. The next practical step is MacBook/Xcode physical-device verification of Phase 21-A3-R3, then compare against `../docs/phase-roadmap-sequencing-and-next-action-register.md` before choosing the next phase.
