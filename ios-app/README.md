...
PT2-SF-R8-RUN ran the approved DEBUG Filter Lab backend smoke exactly once. It used the backend boundary only, sent one in-memory style reference image, sent no apply/original image, and returned sanitized `internal_cloud_disabled` before any provider/model call because the ignored backend debug env gate was closed. iOS runtime remains unchanged: no SiliconFlow endpoint/API key in iOS, no provider SDK, no direct provider call, no payload expansion, no Camera cloud AI entry, and no production rollout. The next safe step is a server-side ignored env gate correction plus fresh approval before any second smoke.
