...
PT2-SF-R8-R1 corrected only the ignored local backend debug env gate after R8-RUN returned `internal_cloud_disabled`. The no-network gate check now resolves Filter Lab to `siliconflowInternal` with the existing debug header. iOS runtime remains unchanged: no SiliconFlow endpoint/API key in iOS, no provider SDK, no direct provider call, no payload expansion, no Camera cloud AI entry, and no production rollout. Any second provider-backed smoke still needs fresh explicit approval.
