...
Phase OD-R1 starts the research / parameter / dataset / fine-tune preparation direction with a backend-only Aesthetic Parameter Registry. Use `npm run qa:aesthetic-parameters:registry` to validate the starter enum/key registry. OD-R1 adds no crawler/download mode, cloud teacher call, provider runtime, model call, training, inference, endpoint, upload path, dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, secret, or production rollout. Cloud AI teacher work is later offline structured labeling only; local CV / local AI remains the future runtime student.

Phase OD-R2 adds backend-only source/image/label-job manifest schemas and validation. Use `npm run qa:aesthetic-dataset:manifest` to validate safe inline synthetic manifests only. OD-R2 adds no crawler/download mode, cloud teacher call, provider runtime, model call, training, inference, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, secret, or production rollout.

Phase OD-R3 adds a backend-only Parameter Mining Bot dry-run. Use `npm run qa:aesthetic-parameters:bot-dry-run` to validate the sanitized in-memory synthetic job plan. OD-R3 connects the OD-R1 registry and OD-R2 manifest validators, but it adds no crawler/download mode, image read, cloud teacher call, provider/model API call, training, inference, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, secret, or production rollout.

Phase OD-R4 adds the backend-only Cloud AI Teacher contract. Use `npm run qa:aesthetic-teacher:contract` to validate the safe inline request/response stub, app-transfer-readiness gates, and sanitized output. OD-R4 is contract-only: no cloud provider call, provider key, network call, image read/upload, crawler/download, training, inference, endpoint, upload path, app runtime transfer, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, secret, or production rollout is added.

Phase OD-R5A adds the backend-only Cloud AI Teacher sandbox preflight and stub adapter. Use `npm run qa:aesthetic-teacher:sandbox-preflight` to validate OD-R3-style sanitized job plans, generate OD-R4-contract-shaped stub responses, and keep all provider/cloud/network/image/training/runtime/app-transfer flags disabled. It adds no provider call, network call, image read/upload, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, secret, or production rollout.

Phase OD-R6A adds the backend-only Parameter Candidate Runner dry-run. Use `npm run qa:aesthetic-parameters:candidate-dry-run` to convert OD-R4/OD-R5A teacher stub labels into structured parameter candidates and aggregate counts. It adds no provider call, network call, image read/upload, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training, app transfer, secret, or production rollout.

Phase OD-R6B adds the backend-only Human Review Queue schema. Use `npm run qa:aesthetic-review:queue` to validate review items and decisions before label/parameter candidates can become calibration, evaluation, or tuning candidates. It adds no provider call, network call, image read/upload, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training, app transfer, secret, or production rollout.

Phase OD-R5B adds a backend-only Cloud Teacher provider sandbox readiness gate and redacted future request envelope. Use `npm run qa:aesthetic-teacher:provider-gate` to validate the disabled example config, fail-closed provider readiness, redaction policy, ignored local config/sample/report boundaries, and no-call guarantees. It adds no provider call, network call, image read/upload, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training, app transfer, secret, or production rollout.

Phase OD-R5C adds a backend-only Cloud Teacher provider smoke harness. Use `npm run qa:aesthetic-teacher:provider-smoke-harness` to validate the default `blocked_no_network` path, OD-R5B gate dependency, redacted envelope usage, OD-R4 contract validation, OD-R6B human review queue routing, disabled adapter, and test-only stub adapter. It adds no real provider implementation, provider SDK/package, provider call, network call, image read/upload, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training, app transfer, secret, or production rollout.

Phase OD-R7A adds a backend-only Local CV Feature Vector Benchmark dry-run. Use `npm run qa:aesthetic-local-cv:feature-benchmark` to validate safe inline synthetic numeric feature vectors against OD-R1 registry keys, OD-R6A parameter candidates, and OD-R6B review decisions. It adds no real image read, CV inference, Python/OpenCV/PyTorch dependency, provider call, network call, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training, app transfer, secret, or production rollout.

Phase OD-R7B adds a backend/local-only Local CV Feature Extractor prototype. Use `npm run qa:aesthetic-local-cv:feature-extractor` to map safe inline synthetic local-CV observations into OD-R7A-compatible feature vectors. It adds no real image read, CV inference, Python/OpenCV/PyTorch dependency, cloud/provider/Xiaoyi relay call, network call, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training/fine-tuning, app transfer, secret, or production rollout.

Phase OD-R7C adds a backend/local-only Local CV calibration fixture gate. Use `npm run qa:aesthetic-local-cv:calibration-fixture-gate` to validate ignored fixture registry policy and approved synthetic/ignored fixture-token shape without reading images by default. It adds no real image read, CV inference, Python/OpenCV/PyTorch dependency, cloud/provider/Xiaoyi relay call, network call, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training/fine-tuning, app transfer, secret, or production rollout.

Phase OD-R7D adds a backend/local-only ignored local fixture calibration smoke. Use `npm run qa:aesthetic-local-cv:calibration-smoke` to run the safely blocked default path, or an explicitly approved ignored-local `calibration_001` fixture smoke when local config and fixture gates pass. It may read one ignored local fixture only after approval gates pass, emits sanitized in-memory feature buckets only, writes no reports, and adds no provider/cloud/Xiaoyi relay call, network call, upload path, endpoint, real dataset/photo/local config/generated report commit, Swift runtime file, model artifact, Core ML package, training/fine-tuning, app transfer, secret, or production rollout.

Phase OD-R7E adds backend/local-only expected range comparison for sanitized OD-R7D calibration output. Use `npm run qa:aesthetic-local-cv:expected-range` to compare the inline sanitized `calibration_001` vector against soft expected ranges. It reads no images by default, runs no CV inference, writes no reports, emits warning buckets rather than scores, and adds no provider/cloud/Xiaoyi relay call, network call, upload path, endpoint, real dataset/photo/local config/generated report commit, Swift runtime file, model artifact, Core ML package, training/fine-tuning, app transfer, secret, or production rollout.

Phase OD-R7F adds backend/local-only Local CV calibration multi-fixture aggregation. Use `npm run qa:aesthetic-local-cv:calibration-aggregation` to aggregate sanitized OD-R7E expected-range outputs into warning frequency and feature status buckets without reading images by default. The current inline fixture count is `1`, so `insufficientSampleSize:true` blocks parameter tuning handoff; it emits no score/rating/aesthetic grade and adds no provider/cloud/Xiaoyi relay call, network/upload, real image read by default, generated report, Swift/Xcode change, model artifact, training/fine-tuning, app runtime transfer, or production rollout.

Phase OD-R7F-R2 updates the aggregation gate to use five manually reviewed sanitized OD-R7F-R1 feature vectors by default. Use `npm run qa:aesthetic-local-cv:calibration-aggregation` to confirm `fixtureCount:5`, `insufficientSampleSize:false`, warning frequency buckets, no blockers, `eligibleForParameterTuning:false`, `eligibleForAppRuntime:false`, and `productionReady:false`. It also fixes/regression-tests safe fixture-root resolution for the OD-R7D smoke CLI without adding image reads by default, report writes, committed fixtures/configs, provider/cloud/network/upload, Swift/Xcode changes, model files, tuning/runtime handoff, or production rollout.

Phase OD-R8A adds a backend-only Parameter Tuning Harness dry-run. Use `npm run qa:aesthetic-parameters:tuning-dry-run` to convert reviewed synthetic parameter candidates plus synthetic local CV benchmark signals into a structured parameter-pack candidate. It adds no real image read, CV inference, Python/OpenCV/PyTorch dependency, provider call, network call, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training/fine-tuning, app transfer, secret, or production rollout.

Phase OD-R8B adds a backend-only Parameter Pack Export Gate. Use `npm run qa:aesthetic-parameters:export-gate` to validate a safe inline synthetic OD-R8A parameter-pack candidate as structurally exportable while keeping app runtime transfer blocked. It adds no provider/cloud call, API key use, network call, image read/upload, CV inference, Python/OpenCV/PyTorch dependency, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, model artifact, Core ML package, training/fine-tuning, app runtime write, app transfer, secret, or production rollout.

Phase OD-R8C adds a backend/local-only Parameter Tuning Aggregation Bridge. Use `npm run qa:aesthetic-parameters:tuning-aggregation-bridge` to convert the sanitized OD-R7F-R2 five-fixture aggregation summary into internal tuning review signals for a future OD-R8D dry-run. It reads no images, runs no CV inference, writes no reports, creates no production parameter pack, exports nothing, changes no endpoint or app runtime, and adds no provider/cloud/Xiaoyi relay call, network/upload, Swift/Xcode change, model artifact, training/fine-tuning, app runtime transfer, secret, or production rollout.

Phase OD-R8D adds a backend/local-only Parameter Tuning Dry-run From Bridge. Use `npm run qa:aesthetic-parameters:tuning-dry-run-from-bridge` to consume OD-R8C bridge output and emit review-only parameter adjustment candidates. It does not export a parameter pack, write reports, read images, run CV inference, change endpoints or app runtime, or add provider/cloud/Xiaoyi relay calls, network/upload, Swift/Xcode changes, model artifacts, training/fine-tuning, secrets, or production rollout.

Phase OD-R8E adds a backend/local-only Reviewed Candidate Acceptance Gate. Use `npm run qa:aesthetic-parameters:candidate-acceptance-gate` to apply manual review decisions to OD-R8D candidates. It accepts `visualWeightMoment.balanceWarningPolicy` and `horizonAngle.softTiltThreshold` for future pack-candidate review, marks `headroomRatio.highSoftWarningThreshold` and `sharpnessRatio.lowConfidenceThreshold` as `needs_more_data`, and keeps parameter-pack export, app runtime, reports, image reads, CV inference, provider/cloud/Xiaoyi relay calls, network/upload, model artifacts, secrets, and production rollout blocked.

Phase OD-R8F adds a backend/local-only Parameter-pack Candidate Builder. Use `npm run qa:aesthetic-parameters:pack-candidate-builder` to build a review-only candidate scaffold from the OD-R8E accepted candidates only. It includes `horizonAngle.softTiltThreshold` and `visualWeightMoment.balanceWarningPolicy`, excludes the two `needs_more_data` candidates, emits internal parameter policy candidates only, and keeps parameter-pack export, app runtime, reports, image reads, CV inference, provider/cloud/Xiaoyi relay calls, network/upload, model artifacts, secrets, and production rollout blocked.

Phase OD-P1A adds a backend-only App Transfer Preflight Gate. Use `npm run qa:aesthetic-parameters:app-transfer-preflight` to validate a safe inline synthetic OD-R8B export candidate against app contract requirements while keeping app runtime transfer blocked. It adds no provider/cloud call, API key use, network call, image read/upload, CV inference, Python/OpenCV/PyTorch dependency, endpoint, upload path, real dataset/photo/local config/generated report, Swift runtime file, Xcode project modification, app bundle write, model artifact, Core ML package, training/fine-tuning, app runtime write, app transfer, secret, or production rollout.
