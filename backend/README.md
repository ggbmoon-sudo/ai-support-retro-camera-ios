...
Current live status: the replacement external/untracked credential passed text and image requests. After R16-R1, one bounded target-only backend request returned validated recipe `2.0` in one provider attempt. The recipe used `contrast:0.06`, `fade:0.04`, `shadowLift:0.02`, exact `0/1` R/G/B endpoints, eight normalized basis weights, and a compound black-floor value of `0.0204` against the `0.035` budget. Its ignored `filter_lab_sanitized_analysis.v2` artifact reports `single_attempt`, all seven privacy flags false, and no raw provider material retained. The current process is reachable locally and at `192.168.68.60:8787` in `xiaoyiLunaInternal` mode with sanitized artifact persistence enabled and `productionReady:false`. The key is injected from its external file for this process only and must be re-injected after a backend restart.
