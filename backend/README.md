...
Local AI Compose P25 uses the explicitly approved internal/debug `POST /v1/ai/composition-planner` path for bounded live-like keyframes. Each schema `1.1` request is one consented, metadata-stripped JPEG plus non-identifying buckets and a top-left permille focus hint. Xiaoyi `gpt-5.6-luna` must return an exact enum plan plus a grounded subject kind/box; free prose, extra fields, scores, sensitive inference, focus inconsistency, and invalid target geometry fail closed. The client permits at most one request in flight and schedules the next frame only after the reply plus at least one second. The route does not persist images/prompts/responses and remains `productionReady:false`. Fast tracking stays on the iPhone; this endpoint is not bidirectional Gemini Live, video streaming, WebSocket, or automatic capture.
...
PT2-SF-R9-R16 upgrades internal/debug Filter Lab generation to strict recipe `2.0`. The backend now validates bounded five-point luma/R/G/B curves, a normalized exact allowlist of eight app-owned basis-look weights, input-normalization/transform intensity, and bounded film-response parameters for grain size/roughness/luma response, halation, and diffusion. Raw LUT data/URLs, arbitrary renderer names, shaders, and code remain forbidden. Sanitized artifact persistence allowlists the normalized v2 recipe only. The request still contains exactly one style/reference image; the apply/original stays on device, and `productionReady:false` remains locked.

PT2-SF-R9-R15 upgrades the internal/debug generated recipe to `1.1`, makes final rendered pixels primary prompt evidence, treats unknown third-party sliders as relative hints only, and adds bounded `shadowLift`, `highlightRollOff`, `bloom`, and `dust` controls. Optional `FILTER_LAB_SAVE_SANITIZED_ANALYSIS=true` persistence writes only an explicit allowlist of validated recipe keys/parameters, confidence bucket, and safe metadata buckets to the ignored `backend/reports/filter-lab-sanitized-analysis/` directory. It never writes the model-generated recipe ID, exact confidence, images/base64, raw prompts/requests/provider responses, Authorization headers, or keys. The setting defaults to false, no live provider call was made in this phase, and `productionReady:false` remains locked.

PT2-SF-R9-R13 calibrates only the internal/debug Filter Lab recipe path. The SiliconFlow request uses a strict JSON schema, content/style separation, and renderer-specific parameter semantics; the no-network fidelity evaluator records only sanitized error buckets. This phase makes zero provider calls, keeps the request at exactly one style/reference image, does not upload the local apply/original image, adds no app-facing production rollout, and keeps `productionReady:false` locked. Model/deployment `json_schema` compatibility still requires a separately approved bounded provider QA run.
...
Phase 21-C-R3-RUN prerequisite check blocked before any benchmark execution because the exact approval phrase was absent, the local ignored Depth Anything artifact folder contained only the placeholder README, and Xcode/physical-device tooling is unavailable in this Windows environment. Backend runtime remains unchanged: no endpoint, provider call, model call, model artifact, Core ML package, model download, inference, benchmark, upload payload change, raw artifact, credential, secret, or production rollout is added.
...
For local development, the key may instead remain in one ignored external text file. Set only its path in the ignored backend environment; the file may contain the raw key or one `XIAOYI_API_KEY=...` line. A direct `XIAOYI_API_KEY` takes precedence, and a missing/invalid file fails closed:

```sh
XIAOYI_API_KEY_FILE=/absolute/path/outside-the-repo/xiaoyi-api-key.txt
```

Restart `npm start`, then confirm `/health` reports `compositionPlannerReady:true` and `productionReady:false`. A physical DEBUG iPhone must still use the computer's private-LAN backend URL and explicit in-app consent for each live session.
...
Current live status: the replacement external/untracked credential passed text and image requests. After R16-R2, one bounded target-only request returned validated recipe `2.0` in one provider attempt. Transfer-aware guidance reduced the prior real recipe's `exposure:-0.02 / contrast:0.08 / vignette:0.12` to `exposure:0.02 / contrast:0.04 / vignette:0.08`, while retaining a `0.255` luma shadow point, warm colour intent, exact `0/1` R/G/B endpoints, and eight normalized basis weights. The ignored `filter_lab_sanitized_analysis.v2` artifact reports `single_attempt`, all seven privacy flags false, and no raw provider material retained. The current process is reachable locally and at `192.168.68.60:8787` in `xiaoyiLunaInternal` mode with sanitized artifact persistence enabled and `productionReady:false`. The key is injected from its external file for this process only and must be re-injected after a backend restart.

R16-R2 polish restart: the latest transferable-tone prompt is loaded in the current `xiaoyiLunaInternal` process. Localhost and `192.168.68.60:8787` health checks report Filter Lab ready and `productionReady:false`. No additional provider request was made for the renderer-only calibration polish.

PT2-SF-R9-R15-X2 real-reference correction: Filter Lab now sends Luna the reference at `detail:high` and explicitly removes screenshot settings/share UI from photographic tone/color evidence. It preserves deep blacks, prevents fade + shadow lift + negative contrast from compounding on bright/flash references, and locks Luna array field types without weakening validation. The accepted post-fix real-reference recipe changed the problematic combination from `contrast:-0.18 / fade:0.20 / shadowLift:0.16` to `contrast:0.01 / fade:0.06 / shadowLift:0.04`. The running backend has been restarted with this prompt; iOS must be rebuilt to receive the separate renderer gain correction.
