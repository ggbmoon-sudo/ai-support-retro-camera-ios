...
Current live status: the replacement external/untracked credential passed text and image requests. After R16, one bounded target-only backend request returned validated recipe `2.0` in one provider attempt with 12 fine controls, four five-point curves, eight normalized basis weights, and the bounded film block. Its ignored artifact is `filter_lab_sanitized_analysis.v2`, all privacy flags are false, and no raw provider material is retained. The current process is reachable locally and at `192.168.68.60:8787` in `xiaoyiLunaInternal` mode with sanitized artifact persistence enabled and `productionReady:false`. The key is injected from its external file for this process only and must be re-injected after a backend restart.
